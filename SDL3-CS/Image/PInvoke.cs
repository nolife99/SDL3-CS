...
public partial class Image
{
    ...
    /// <summary> This function gets the version of the dynamically linked SDL_image library. </summary>
    ...
    /// <para>
    /// An <see cref="Surface"/> is a buffer of pixels in memory accessible by the CPU. Use this if you plan to hand the data
    /// to something else or manipulate it further in code.
    /// </para>
    /// <para>
    /// There are no guarantees about what format the new <see cref="Surface"/> data will be; in many cases, SDL_image will
    /// attempt to supply a surface that exactly matches the provided image, but in others it might have to convert (either because
    /// the image is in a format that SDL doesn't directly support or because it's compressed data that could reasonably uncompress
    /// to various formats and SDL_image had to pick one). You can inspect an <see cref="Surface"/> for its specifics, and use
    /// <see cref="SDL.ConvertSurface"/> to then migrate to any supported format.
    /// </para>
    /// <para>
    /// If the image format supports a transparent pixel, SDL will set the colorkey for the surface. You can enable RLE
    /// acceleration on the surface afterwards by calling: SDL.SetSurfaceColorKey(image, SDL_RLEACCEL, image.Format.Colorkey);
    /// </para>
    /// <para>
    /// If <c> closeio </c> is true, <c> src </c> will be closed before returning, whether this function succeeds or not.
    /// SDL_image reads everything it needs from <c> src </c> during this call in any case.
    /// </para>
    /// <para>
    /// Even though this function accepts a file type, SDL_image may still try other decoders that are capable of detecting
    /// file type from the contents of the image data, but may rely on the caller-provided type string for formats that it cannot
    /// autodetect. If <c> type </c> is <c> null </c>, SDL_image will rely solely on its ability to guess the format.
    /// </para>
    /// <para>
    /// There is a separate function to read files from disk without having to deal with SDL_IOStream:
    /// <c> Load("filename.jpg") </c> will call this function and manage those details for you, determining the file type from the
    /// filename's extension.
    /// </para>
    /// <para>
    /// There is also <see cref="LoadIO"/>, which is equivalent to this function except that it will rely on SDL_image to
    /// determine what type of data it is loading, much like passing a <c> null </c> for type.
    /// </para>
    /// <para>
    /// If you are using SDL's 2D rendering API, there is an equivalent call to load images directly into an SDL_Texture for
    /// use by the GPU without using a software surface: call <see cref="LoadTextureTypedIO"/> instead.
    /// </para>
    /// <para> When done with the returned surface, the app should dispose of it with a call to <see cref="SDL.DestroySurface"/> . </para>
    /// </summary>
    /// <param name="src"> an SDL_IOStream that data will be read from. </param>
    /// <param name="closeio"> <c> true </c> to close/free the SDL_IOStream before returning, <c> false </c> to leave it open. </param>
    ...
    public static unsafe SurfaceHandle LoadTypedIO(IOStreamHandle src, bool closeio, scoped ReadOnlySpan<char> type)
    {
        using var u = new Utf8(type, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_LoadTypedIO(src, closeio, (nint)p);
    }

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadTyped_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial SurfaceHandle SDL_LoadTypedIO(IOStreamHandle src,
    ...
    /// </para>
    /// <para>
    /// There is a separate function to read files from an SDL_IOStream, if you need an i/o abstraction to provide data from
    /// anywhere instead of a simple filesystem read; that function is <see cref="LoadIO"/>.
    /// </para>
    /// <para>
    /// If you are using SDL's 2D rendering API, there is an equivalent call to load images directly into an SDL_Texture for
    /// use by the GPU without using a software surface: call <see cref="LoadTexture"/> instead.
    /// </para>
    /// <para>
    ...
    /// <seealso cref="LoadIO"/>
    /// <seealso cref="SDL.DestroySurface"/>
    public static unsafe SurfaceHandle Load(scoped ReadOnlySpan<char> file)
    {
        using var u = new Utf8(file, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_Load((nint)p);
    }

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_Load"), UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial SurfaceHandle SDL_Load(nint file);
    ...
    /// <para>
    /// There is a separate function to read files from disk without having to deal with SDL_IOStream:
    /// <c> Image.Load("filename.jpg") </c> will call this function and manage those details for you, determining the file type from
    /// the filename's extension.
    /// </para>
    /// <para>
    /// There is also <see cref="LoadTypedIO"/>, which is equivalent to this function except a file extension (like "BMP",
    /// "JPG", etc) can be specified, in case SDL_image cannot autodetect the file format.
    /// </para>
    /// <para>
    /// If you are using SDL's 2D rendering API, there is an equivalent call to load images directly into an SDL_Texture for
    /// use by the GPU without using a software surface: call <see cref="LoadTextureIO"/> instead.
    /// </para>
    /// <para> When done with the returned surface, the app should dispose of it with a call to <see cref="SDL.DestroySurface"/> . </para>
    /// </summary>
    /// <param name="src"> an SDL_IOStream that data will be read from. </param>
    ...
    public static partial SurfaceHandle LoadIO(IOStreamHandle src, [MarshalAs(UnmanagedType.I1)] bool closeio);
    ...
    /// <para>
    /// An SDL_Texture represents an image in GPU memory, usable by SDL's 2D Render API. This can be significantly more
    /// efficient than using a CPU-bound <see cref="Surface"/>  if you don't need to manipulate the image directly after loading it.
    /// </para>
    /// <para>
    /// If the loaded image has transparency or a colorkey, a texture with an alpha channel will be created. Otherwise,
    /// SDL_image will attempt to create an SDL_Texture in the most format that most reasonably represents the image data (but in
    /// many cases, this will just end up being 32-bit RGB or 32-bit RGBA).
    /// </para>
    /// <para>
    /// There is a separate function to read files from an SDL_IOStream, if you need an i/o abstraction to provide data from
    /// anywhere instead of a simple filesystem read; that function is <see cref="LoadTextureIO"/>.
    /// </para>
    /// <para>
    /// If you would rather decode an image to an <see cref="Surface"/>  (a buffer of pixels in CPU memory), call
    /// <see cref="Load"/> instead.
    /// </para>
    /// <para> When done with the returned texture, the app should dispose of it with a call to <see cref="SDL.DestroyTexture"/> . </para>
    /// </summary>
    ...
    public static unsafe nint LoadTexture(nint renderer, scoped ReadOnlySpan<char> file)
    {
        using var u = new Utf8(file, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_LoadTexture(renderer, (nint)p);
    }

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadTexture"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial nint SDL_LoadTexture(nint renderer, nint file);
    ...
    /// <para>
    /// There is a separate function to read files from disk without having to deal with SDL_IOStream:
    /// <c> LoadTexture(renderer, "filename.jpg") </c> will call this function and manage those details for you, determining the
    /// file type from the filename's extension.
    /// </para>
    /// <para>
    /// There is also <see cref="LoadTextureTypedIO"/>, which is equivalent to this function except a file extension (like
    /// "BMP", "JPG", etc) can be specified, in case SDL_image cannot autodetect the file format.
    /// </para>
    /// <para>
    /// If you would rather decode an image to an SDL_Surface (a buffer of pixels in CPU memory), call <see cref="Load"/>
    /// instead.
    /// </para>
    /// <para> When done with the returned texture, the app should dispose of it with a call to <see cref="SDL.DestroyTexture"/> . </para>
    /// </summary>
    ...
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadTexture_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial nint LoadTextureIO(nint renderer, IOStreamHandle src, [MarshalAs(UnmanagedType.I1)] bool closeio);
    ...
    /// <para>
    /// An SDL_Texture represents an image in GPU memory, usable by SDL's 2D Render API. This can be significantly more
    /// efficient than using a CPU-bound SDL_Surface if you don't need to manipulate the image directly after loading it.
    /// </para>
    /// <para>
    /// If the loaded image has transparency or a colorkey, a texture with an alpha channel will be created. Otherwise,
    /// SDL_image will attempt to create an SDL_Texture in the most format that most reasonably represents the image data (but in
    ...
    /// <para>
    /// There is a separate function to read files from disk without having to deal with SDL_IOStream:
    /// <c> Image.LoadTexture("filename.jpg") </c> will call this function and manage those details for you, determining the file
    /// type from the filename's extension.
    /// </para>
    /// <para>
    /// There is also <see cref="LoadTextureIO"/>, which is equivalent to this function except that it will rely on SDL_image
    /// to determine what type of data it is loading, much like passing a <c> null </c> for type.
    /// </para>
    /// <para>
    /// If you would rather decode an image to an SDL_Surface (a buffer of pixels in CPU memory), call
    /// <see cref="LoadTypedIO"/> instead.
    /// </para>
    /// <para> When done with the returned texture, the app should dispose of it with a call to <see cref="SDL.DestroyTexture"/> . </para>
    /// </summary>
    /// <param name="renderer"> the SDL_Renderer to use to create the GPU texture. </param>
    ...
    /// <seealso cref="LoadTextureIO"/>
    /// <seealso cref="SDL.DestroyTexture"/>
    public static unsafe nint LoadTextureTypedIO(nint renderer, IOStreamHandle src, bool closeio,
        scoped ReadOnlySpan<char> type)
    {
        using var u = new Utf8(type, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_LoadTextureTypedIO(renderer, src, closeio, (nint)p);
    }

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadTextureTyped_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial nint SDL_LoadTextureTypedIO(nint renderer,
        IOStreamHandle src,
        [MarshalAs(UnmanagedType.I1)] bool closeio,
        nint type);
    ...
    /// <para>
    /// This function attempts to determine if a file is a given filetype, reading the least amount possible from the
    /// SDL_IOStream (usually a few bytes).
    /// </para>
    /// <para> There is no distinction made between "not the filetype in question" and basic i/o errors. </para>
    /// <para>
    /// This function will always attempt to seek <c> src </c> back to where it started when this function was called, but it
    /// will not report any errors in doing so, but assuming seeking works, this means you can immediately use this with a different
    /// IsTYPE function, or load the image without further seeking.
    /// </para>
    /// <para>
    /// You do not need to call this function to load data; SDL_image can work to determine file type in many cases in its
    ...
    public static partial bool IsAVIF(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsICO(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsCUR(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsBMP(IOStreamHandle src);
    ...
    /// <para> There is no distinction made between "not the filetype in question" and basic i/o errors. </para>
    /// <para>
    /// This function will always attempt to seek `src` back to where it started when this function was called, but it will
    /// not report any errors in doing so, but assuming seeking works, this means you can immediately use this with a different
    /// IsTYPE function, or load the image without further seeking.
    /// </para>
    /// <para>
    /// You do not need to call this function to load data; SDL_image can work to determine file type in many cases in its
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsGIF(IOStreamHandle src);
    ...
    /// This function will always attempt to seek <c> src </c> back to where it started when this function was called, but it
    /// will not report any errors in doing so, but assuming seeking works, this means you can immediately use this with a different
    /// IMG_isTYPE function, or load the image without further seeking.
    /// </para>
    /// <para>
    /// You do not need to call this function to load data; SDL_image can work to determine file type in many cases in its
    /// standard load functions.
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsJPG(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsJXL(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsLBM(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsPCX(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsPNG(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsPNM(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsSVG(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsQOI(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsTIF(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsXCF(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsXPM(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsXV(IOStreamHandle src);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool IsWEBP(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadAVIF_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a AVIF image directly. </para>
    /// <para>
    /// If you know you definitely have a AVIF image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadAVIF_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial SurfaceHandle LoadAVIFIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadICO_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a ICO image directly. </para>
    /// <para>
    /// If you know you definitely have a ICO image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadICOIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadCUR_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a CUR image directly. </para>
    /// <para>
    /// If you know you definitely have a CUR image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadCURIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadBMP_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a BMP image directly. </para>
    /// <para>
    /// If you know you definitely have a BMP image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadBMPIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadGIF_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a GIF image directly. </para>
    /// <para>
    /// If you know you definitely have a GIF image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadGIFIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadJPG_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a JPG image directly. </para>
    /// <para>
    /// If you know you definitely have a JPG image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadJPGIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadJXL_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a JXL image directly. </para>
    /// <para>
    /// If you know you definitely have a JXL image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadJXLIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadLBM_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a LBM image directly. </para>
    /// <para>
    /// If you know you definitely have a LBM image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadLBMIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadPCX_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a PCX image directly. </para>
    /// <para>
    /// If you know you definitely have a PCX image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadPCXIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadPNG_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a PNG image directly. </para>
    /// <para>
    /// If you know you definitely have a PNG image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadPNGIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadPNM_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a PNM image directly. </para>
    /// <para>
    /// If you know you definitely have a PNM image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadPNMIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadSVG_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a SVG image directly. </para>
    /// <para>
    /// If you know you definitely have a SVG image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadSVGIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadQOI_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a QOI image directly. </para>
    /// <para>
    /// If you know you definitely have a QOI image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadQOIIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadTGA_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a TGA image directly. </para>
    /// <para>
    /// If you know you definitely have a TGA image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadTGAIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadTIF_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a TIFF image directly. </para>
    /// <para>
    /// If you know you definitely have a TIFF image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadTIFIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadXCF_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a XCF image directly. </para>
    /// <para>
    /// If you know you definitely have a XCF image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadXCFIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadXPM_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a XPM image directly. </para>
    /// <para>
    /// If you know you definitely have a XPM image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadXPMIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadXV_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a XV image directly. </para>
    /// <para>
    /// If you know you definitely have a XV image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    public static partial SurfaceHandle LoadXVIO(IOStreamHandle src);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_LoadWEBP_IO(SDL_IOStream *src);</code>
    /// <summary>
    /// <para> Load a WEBP image directly. </para>
    /// <para>
    /// If you know you definitely have a WEBP image, you can call this function, which will skip SDL_image's file format
    /// detection routines. Generally it's better to use the abstract interfaces; also, there is only an SDL_IOStream interface
    /// available here.
    /// </para>
    /// </summary>
    ...
    /// <seealso cref="LoadXPMIO"/>
    /// <seealso cref="LoadXVIO"/>
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadWEBP_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial SurfaceHandle LoadWEBPIO(IOStreamHandle src);
    ...
    /// <para>
    /// Since SVG files are resolution-independent, you specify the size you would like the output image to be and it will be
    /// generated at those dimensions.
    /// </para>
    ...
    /// <returns> a new SDL surface, or <c> null </c> on error. </returns>
    /// <since> This function is available since SDL_image 3.0.0. </since>
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadSizedSVG_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial SurfaceHandle LoadSizedSVGIO(IOStreamHandle src, int width, int height);
    ...
    /// <para>
    /// The returned surface will be an 8bpp indexed surface, if possible, otherwise it will be 32bpp. If you always want
    /// 32-bit data, use <see cref="ReadXPMFromArrayToRGB888"/> instead.
    /// </para>
    /// <para> When done with the returned surface, the app should dispose of it with a call to <see cref="SDL.DestroySurface"/> . </para>
    /// </summary>
    ...
    /// <returns> a new SDL surface, or <c> null </c> on error. </returns>
    /// <since> This function is available since SDL_image 3.0.0. </since>
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_ReadXPMFromArray"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial SurfaceHandle ReadXPMFromArray(
        [MarshalAs(UnmanagedType.LPArray, ArraySubType = UnmanagedType.LPUTF8Str)] string[] xpm);

    /// <code>extern SDL_DECLSPEC SDL_Surface * SDLCALL IMG_ReadXPMFromArrayToRGB888(char **xpm);</code>
    /// <summary>
    /// <para> Load an XPM image from a memory array. </para>
    /// <para>
    /// The returned surface will always be a 32-bit RGB surface. If you want 8-bit indexed colors (and the XPM data allows
    /// it), use <see cref="ReadXPMFromArray"/> instead.
    /// </para>
    /// <para> When done with the returned surface, the app should dispose of it with a call to <see cref="SDL.DestroySurface"/> . </para>
    /// </summary>
    /// <param name="xpm"> a null-terminated array of strings that comprise XPM data. </param>
    ...
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_ReadXPMFromArrayToRGB888"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial SurfaceHandle ReadXPMFromArrayToRGB888(
        [MarshalAs(UnmanagedType.LPArray, ArraySubType = UnmanagedType.LPUTF8Str)] string[] xpm);
    ...
    public static unsafe bool SaveAVIF(SurfaceHandle surface, scoped ReadOnlySpan<char> file, int quality)
    {
        using var u = new Utf8(file, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_SaveAVIF(surface, (nint)p, quality);
    }

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_SaveAVIF"), UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    private static partial bool SDL_SaveAVIF(SurfaceHandle surface, nint file, int quality);
    ...
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_SaveAVIF_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool SaveAVIFIO(SurfaceHandle surface,
        IOStreamHandle dst,
        [MarshalAs(UnmanagedType.I1)] bool closeio,
        int quality);
    ...
    /// <summary>
    /// <para> Save an SDL_Surface into a PNG image file. </para> <para> If the file already exists, it will be overwritten. </para>
    /// </summary>
    /// <param name="surface"> the SDL surface to save. </param>
    /// <param name="file"> path on the filesystem to write new file to. </param>
    /// <returns> <c> true </c> on success or <c> false </c> on failure; call <see cref="SDL.GetError"/> for more information. </returns>
    ...
    public static unsafe bool SavePNG(SurfaceHandle surface, scoped ReadOnlySpan<char> file)
    {
        using var u = new Utf8(file, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_SavePNG(surface, (nint)p);
    }

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_SavePNG"), UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    private static partial bool SDL_SavePNG(SurfaceHandle surface, nint file);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool SavePNGIO(SurfaceHandle surface, IOStreamHandle dst, [MarshalAs(UnmanagedType.I1)] bool closeio);
    ...
    public static unsafe bool SaveJPG(SurfaceHandle surface, scoped ReadOnlySpan<char> file, int quality)
    {
        using var u = new Utf8(file, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_SaveJPG(surface, (nint)p, quality);
    }

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_SaveJPG"), UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    private static partial bool SDL_SaveJPG(SurfaceHandle surface, nint file, int quality);
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool SaveJPGIO(SurfaceHandle surface,
        IOStreamHandle dst,
        [MarshalAs(UnmanagedType.I1)] bool closeio,
        int quality);
    ...
    /// <para> When done with the returned animation, the app should dispose of it with a call to <see cref="FreeAnimation"/>. </para>
    /// </summary>
    ...
    public static unsafe AnimationHandle LoadAnimation(scoped ReadOnlySpan<char> file)
    {
        using var u = new Utf8(file, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_LoadAnimation((nint)p);
    }

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadAnimation"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial AnimationHandle SDL_LoadAnimation(nint file);
    ...
    /// <since> This function is available since SDL_image 3.0.0. </since>
    /// <seealso cref="FreeAnimation"/>
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadAnimation_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial AnimationHandle LoadAnimationIO(IOStreamHandle src, [MarshalAs(UnmanagedType.I1)] bool closeio);
    ...
    /// <seealso cref="FreeAnimation"/>
    public static unsafe AnimationHandle LoadAnimationTypedIO(IOStreamHandle src, bool closeio,
        scoped ReadOnlySpan<char> type)
    {
        using var u = new Utf8(type, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_LoadAnimationTypedIO(src, closeio, (nint)p);
    }

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadAnimationTyped_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial AnimationHandle SDL_LoadAnimationTypedIO(IOStreamHandle src,
        [MarshalAs(UnmanagedType.I1)] bool closeio,
        nint type);
    ...
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_FreeAnimation"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial void FreeAnimation(AnimationHandle anim);
    ...
    /// <seealso cref="LoadAnimationTypedIO"/>
    /// <seealso cref="FreeAnimation"/>
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadGIFAnimation_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial AnimationHandle LoadGIFAnimationIO(IOStreamHandle src);
    ...
    /// <seealso cref="LoadAnimationTypedIO"/>
    /// <seealso cref="FreeAnimation"/>
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_LoadWEBPAnimation_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial AnimationHandle LoadWEBPAnimationIO(IOStreamHandle src);

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_CreateAnimationEncoder"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial AnimationEncoderHandle SDL_CreateAnimationEncoder(nint file);

    /// <code>extern SDL_DECLSPEC IMG_AnimationEncoder * SDLCALL IMG_CreateAnimationEncoder(const char *file);</code>
    /// <summary> Create an animation encoder that writes frames to <paramref name="file"/>; the format is chosen from the file extension. </summary>
    /// <param name="file"> the path to save the animation to. </param>
    /// <returns> a new encoder, or <see cref="AnimationEncoderHandle.Null"/> on failure; call <see cref="SDL.GetError"/> for more information. </returns>
    ...
    public static unsafe AnimationEncoderHandle CreateAnimationEncoder(scoped ReadOnlySpan<char> file)
    {
        using var u = new Utf8(file, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_CreateAnimationEncoder((nint)p);
    }

    [LibraryImport(ImageLibrary, EntryPoint = "IMG_CreateAnimationEncoder_IO"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial AnimationEncoderHandle SDL_CreateAnimationEncoderIO(IOStreamHandle dst,
        [MarshalAs(UnmanagedType.I1)] bool closeio,
        nint type);
    ...
    /// <summary> Create an animation encoder that writes frames to an <see cref="IOStreamHandle"/>. </summary>
    /// <param name="dst"> the stream to write the animation to. </param>
    /// <param name="closeio"> true to close <paramref name="dst"/> when the encoder is closed, even on failure. </param>
    /// <param name="type"> the output file type (e.g. "gif", "webp", "avif"). </param>
    /// <returns> a new encoder, or <see cref="AnimationEncoderHandle.Null"/> on failure; call <see cref="SDL.GetError"/> for more information. </returns>
    /// <since> This function is available since SDL_image 3.4.0. </since>
    ...
    /// <seealso cref="AddAnimationEncoderFrame"/>
    /// <seealso cref="CloseAnimationEncoder"/>
    public static unsafe AnimationEncoderHandle CreateAnimationEncoderIO(IOStreamHandle dst,
        bool closeio,
        scoped ReadOnlySpan<char> type)
    {
        using var u = new Utf8(type, stackalloc byte[512]);
        fixed (byte* p = u.Bytes) return SDL_CreateAnimationEncoderIO(dst, closeio, (nint)p);
    }
    ...
    /// <summary>
    ///     Create an animation encoder from a property group. See <see cref="EncoderProps"/> for the accepted property names
    ///     (output target, type, quality, timebase, and format-specific options).
    /// </summary>
    /// <param name="props"> the properties describing the encoder to create. </param>
    /// <returns> a new encoder, or <see cref="AnimationEncoderHandle.Null"/> on failure; call <see cref="SDL.GetError"/> for more information. </returns>
    /// <since> This function is available since SDL_image 3.4.0. </since>
    /// <seealso cref="CreateAnimationEncoder"/>
    /// <seealso cref="CreateAnimationEncoderIO"/>
    /// <seealso cref="AddAnimationEncoderFrame"/>
    /// <seealso cref="CloseAnimationEncoder"/>
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_CreateAnimationEncoderWithProperties"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial AnimationEncoderHandle CreateAnimationEncoderWithProperties(PropertiesID props);
    ...
    /// <summary> Append a frame to the animation being encoded. </summary>
    /// <param name="encoder"> the encoder to add a frame to. </param>
    /// <param name="surface"> the pixel data for this frame. </param>
    /// <param name="duration"> the duration of the frame, in the encoder's timebase units. </param>
    /// <returns> <c> true </c> on success or <c> false </c> on failure; call <see cref="SDL.GetError"/> for more information. </returns>
    /// <since> This function is available since SDL_image 3.4.0. </since>
    /// <seealso cref="CreateAnimationEncoder"/>
    /// <seealso cref="CreateAnimationEncoderIO"/>
    /// <seealso cref="CreateAnimationEncoderWithProperties"/>
    /// <seealso cref="CloseAnimationEncoder"/>
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_AddAnimationEncoderFrame"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool AddAnimationEncoderFrame(AnimationEncoderHandle encoder, SurfaceHandle surface, ulong duration);

    /// <code>extern SDL_DECLSPEC bool SDLCALL IMG_CloseAnimationEncoder(IMG_AnimationEncoder *encoder);</code>
    /// <summary> Finish encoding, flush any pending data, free the encoder, and (if requested at creation) close the output stream. </summary>
    /// <param name="encoder"> the encoder to finalize and free. </param>
    /// <returns> <c> true </c> on success or <c> false </c> on failure; call <see cref="SDL.GetError"/> for more information. </returns>
    /// <since> This function is available since SDL_image 3.4.0. </since>
    /// <seealso cref="CreateAnimationEncoder"/>
    /// <seealso cref="CreateAnimationEncoderIO"/>
    /// <seealso cref="CreateAnimationEncoderWithProperties"/>
    [LibraryImport(ImageLibrary, EntryPoint = "IMG_CloseAnimationEncoder"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool CloseAnimationEncoder(AnimationEncoderHandle encoder);
}