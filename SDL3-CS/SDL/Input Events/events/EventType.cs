...
public enum EventType : uint
{
    ...
    /// <summary>
    /// The application is being terminated by the OS. This event must be handled in a callback set with
    /// <see cref="SDL.AddEventWatch"/>. Called on iOS in applicationWillTerminate() Called on Android in onDestroy()
    /// </summary>
    Terminating,

    /// <summary>
    /// The application is low on memory, free memory if possible. This event must be handled in a callback set with
    /// <see cref="SDL.AddEventWatch"/>. Called on iOS in applicationDidReceiveMemoryWarning() Called on Android in onTrimMemory()
    /// </summary>
    LowMemory,

    /// <summary>
    /// The application is about to enter the background. This event must be handled in a callback set with
    /// <see cref="SDL.AddEventWatch"/>. Called on iOS in applicationWillResignActive() Called on Android in onPause()
    /// </summary>
    WillEnterBackground,

    /// <summary>
    /// The application did enter the background and may not get CPU for some time. This event must be handled in a
    /// callback set with <see cref="SDL.AddEventWatch"/>. Called on iOS in applicationDidEnterBackground() Called on Android in
    /// onPause()
    /// </summary>
    DidEnterBackground,

    /// <summary>
    /// The application is about to enter the foreground. This event must be handled in a callback set with
    /// <see cref="SDL.AddEventWatch"/>. Called on iOS in applicationWillEnterForeground() Called on Android in onResume()
    /// </summary>
    WillEnterForeground,

    /// <summary>
    /// The application is now interactive. This event must be handled in a callback set with
    /// <see cref="SDL.AddEventWatch"/>. Called on iOS in applicationDidBecomeActive() Called on Android in onResume()
    /// </summary>
    DidEnterForeground,
    ...
    /// <summary> Window had a hit test that wasn't <see cref="HitTestResult.Normal"/> </summary>
    WindowHitTest,
    ...
    /// <summary>
    /// The window with the associated ID is being or has been destroyed. If this message is being handled in an event
    /// watcher, the window handle is still valid and can still be used to retrieve any properties associated with the window.
