...
public static partial class SDL
{
    ...
    /// <para>
    /// <see cref="PumpEvents"/> gathers all the pending input information from devices and places it in the event queue.
    /// Without calls to <see cref="PumpEvents"/> no events would ever be placed on the queue. Often the need for calls to
    /// <see cref="PumpEvents"/> is hidden from the user since <see cref="PollEvent"/> and <see cref="WaitEvent"/> implicitly call
    /// <see cref="PumpEvents"/>. However, if you are not polling or waiting for events (e.g. you are filtering them), then you must
    /// call <see cref="PumpEvents"/> to force an event queue update.
    /// </para>
    /// </summary>
    ...
    /// <list type="bullet">
    /// <item> <see cref="EventAction.AddEvent"/>: up to <c> numevents </c> events will be added to the back of the event queue. </item>
    /// <item>
    /// <see cref="EventAction.PeekEvent"/>: <c> numevents </c> events at the front of the event queue, within the specified
    /// minimum and maximum type, will be returned to the caller and will <b> not </b> be removed from the queue. If you pass
    /// <c> null </c> for
    /// </item>
    /// <item> <c> events </c>, then <c> numevents </c> is ignored and the total number of matching events will be returned. </item>
    /// <item>
    /// <see cref="EventAction.GetEvent"/>: up to <c> numevents </c> events at the front of the event queue, within the
    /// specified minimum and maximum type, will be returned to the caller and will be removed from the queue.
    /// </item>
    /// </list>
    /// <para>
    /// You may have to call <see cref="PumpEvents"/> before calling this function. Otherwise, the events may not be ready to
    /// be filtered when you call <see cref="PeepEvents(nint, int, EventAction, uint, uint)"/>.
    /// </para>
    /// </summary>
    /// <param name="events">
    /// destination buffer for the retrieved events, may be <c> null </c> to leave the events in the queue and
    /// return the number of events that would have been stored.
    /// </param>
    /// <param name="numevents">
    /// if action is <see cref="EventAction.AddEvent"/>, the number of events to add back to the event
    /// queue; if action is <see cref="EventAction.PeekEvent"/> or <see cref="EventAction.GetEvent"/>, the maximum number of events
    /// to retrieve.
    /// </param>
    /// <param name="action"> action to take; see Remarks for details. </param>
    /// <param name="minType"> minimum value of the event type to be considered; <see cref="EventType.First"/> is a safe choice. </param>
    /// <param name="maxType"> maximum value of the event type to be considered; <see cref="EventType.Last"/> is a safe choice. </param>
    ...
    /// <returns>
    /// <c> true </c> if events matching <c> type </c> are present, or <c> false </c> if events matching <c> type </c> are
    /// not present.
    /// </returns>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    /// </summary>
    /// <param name="minType"> the low end of event type to be queried, inclusive; see <see cref="EventType"/> for details. </param>
    /// <param name="maxType"> the high end of event type to be queried, inclusive; see <see cref="EventType"/> for details. </param>
    /// <returns> <c> true </c> if events with type >= <c> minType </c> and &lt;= <c> maxType </c> are present, or false if not. </returns>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    /// <para>
    /// This will unconditionally remove any events from the queue that match <c> type </c>. If you need to remove a range of
    /// event types, use <see cref="FlushEvents"/> instead.
    /// </para>
    ...
    /// you can call <see cref="PumpEvents"/> on the main thread immediately before the flush call.
    /// </para>
    /// <para>
    /// If you have user events with custom data that needs to be freed, you should use
    /// <see cref="PeepEvents(nint, int, EventAction, uint, uint)"/> to remove and clean up those events before calling this
    /// function.
    /// </para>
    /// </summary>
    ...
    /// <c> maxType </c>, inclusive. If you need to remove a single event type, use <see cref="FlushEvent"/> instead.
    /// </para>
    /// <para> It's also normal to just ignore events you don't care about in your event loop without calling this function. </para>
    /// <para>
    /// This function only affects currently queued events. If you want to make sure that all pending OS events are flushed,
    ...
    /// </para>
    /// </summary>
    /// <param name="minType"> the low end of event type to be cleared, inclusive; see <see cref="EventType"/> for details. </param>
    /// <param name="maxType"> the high end of event type to be cleared, inclusive; see <see cref="EventType"/> for details. </param>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    /// <para>
    /// If <c> event </c> is not <c> null </c>, the next event is removed from the queue and stored in the SDL_Event
    /// structure pointed to by `event`. The 1 returned refers to this event, immediately stored in the SDL Event structure -- not
    /// an event to follow.
    /// </para>
    /// <para>
    /// As this function may implicitly call <see cref="PumpEvents"/>, you can only call this function in the thread that set
    /// the video mode.
    /// </para>
    /// <para>
    /// <see cref="PollEvent"/> is the favored way of receiving system events since it can be done from the main loop and
    /// does not suspend the main loop while waiting on an event to be posted.
    /// </para>
    /// <para>
    /// The common practice is to fully process the event queue once every frame, usually as a first step before updating the
    /// game's state:
    /// </para>
    /// <code>
    ...
    /// </code>
    /// </summary>
    /// <param name="event"> the <see cref="Event"/> structure to be filled with the next event from the queue, or <c> null </c>. </param>
    ...
    /// <para>
    /// If <c> event </c> is not <c> null </c>, the next event is removed from the queue and stored in the
    /// <see cref="Event"/> structure pointed to by <c> event </c>.
    /// </para>
    /// <para>
    /// As this function may implicitly call <see cref="PumpEvents"/>, you can only call this function in the thread that
    /// initialized the video subsystem.
    /// </para>
    /// </summary>
    /// <param name="event"> the <see cref="Event"/> structure to be filled in with the next event from the queue, or <c> null </c>. </param>
    /// <returns>
    /// true on success or false if there was an error while waiting for events; call <see cref="GetError"/> for more
    /// information.
    /// </returns>
    /// <threadsafety> This function should only be called on the main thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    /// <para>
    /// If <c> event </c> is not <c> null </c>, the next event is removed from the queue and stored in the
    /// <seealso cref="Event"/> structure pointed to by <c> event </c>.
    /// </para>
    /// <para>
    /// As this function may implicitly call <see cref="PumpEvents"/>, you can only call this function in the thread that
    /// initialized the video subsystem.
    ...
    /// <returns> <c> true </c> if this got an event or <c> false </c> if the timeout elapsed without any events available. </returns>
    /// <threadsafety> This function should only be called on the main thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="PollEvent"/>
    /// <seealso cref="PushEvent"/>
    ...
    /// but the user can also push their own events onto it. <c> event </c> is a pointer to the event structure you wish to push
    /// onto the queue. The event is copied into the queue, and the caller may dispose of the memory pointed to after
    ...
    /// <para>
    /// Note: Events pushed onto the queue with <see cref="PushEvent"/> get passed through the event filter but events added
    /// with <see cref="PeepEvents(nint, int, EventAction, uint, uint)"/> do not.
    /// </para>
    /// <para>
    /// For pushing application-specific events, please use <see cref="RegisterEvents"/> to get an event type that does not
    ...
    /// <returns>
    /// <c> true </c> on success, <c> false </c> if the event was filtered or on failure; call <see cref="GetError"/> for
    /// more information. A common reason for error is the event queue being full.
    /// </returns>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    /// <see cref="AddEventWatch"/> instead.
    /// </para>
    /// <para>
    /// If the filter function returns true when called, then the event will be added to the internal queue. If it returns
    /// false, then the event will be dropped from the queue, but the internal state will still be updated. This allows selective
    /// filtering of dynamically arriving events.
    /// </para>
    /// <para> <b> WARNING </b>: Be very careful of what you do in the event filter function, as it may run in a different thread! </para>
    /// <para>
    /// On platforms that support it, if the quit event is generated by an interrupt signal (e.g. pressing Ctrl-C), it will
    /// be delivered to the application at the next event poll.
    /// </para>
    /// <para>
    /// There is one caveat when dealing with the <see cref="QuitEvent"/> event type. The event filter is only called when
    /// the window manager desires to close the application window. If the event filter returns 1, then the window will be closed,
    /// otherwise the window will remain open if possible.
    /// </para>
    /// <para> Note: Disabled events never make it to the event filter function; see <see cref="SetEventEnabled"/>. </para>
    /// <para> Note: If you just want to inspect events without filtering, you should use <see cref="AddEventWatch"/> instead. </para>
    /// <para>
    /// Note: Events pushed onto the queue with <see cref="PushEvent"/> get passed through the event filter, but events
    /// pushed onto the queue with <see cref="PeepEvents(nint, int, EventAction, uint, uint)"/> do not.
    /// </para>
    /// </summary>
    ...
    /// <para>
    /// This function can be used to "chain" filters, by saving the existing filter before replacing it with a function that
    /// will call that saved filter.
    /// </para>
    /// </summary>
    ...
    /// <para> <b> WARNING </b>: Be very careful of what you do in the event filter function, as it may run in a different thread! </para>
    /// <para>
    /// If the quit event is generated by a signal (e.g. SIGINT), it will bypass the internal queue and be delivered to the
    /// watch callback immediately, and arrive at the next event poll.
    /// </para>
    /// <para>
    /// Note: the callback is called for events posted by the user through <see cref="PushEvent"/>, but not for disabled
    /// events, nor for events by a filter callback set with <see cref="SetEventFilter"/>, nor for events posted by the user through
    /// <see cref="PeepEvents(nint, int, EventAction, uint, uint)"/>.
    /// </para>
    /// </summary>
    /// <param name="filter"> an <see cref="EventFilter"/> function to call when an event happens. </param>
    /// <param name="userdata"> a pointer that is passed to <c> filter </c>. </param>
    ...
    /// <para> This function takes the same input as <see cref="AddEventWatch"/> to identify and delete the corresponding callback. </para>
    /// </summary>
    ...
    /// <para>
    /// See <see cref="SetEventFilter"/> for more information. Unlike <see cref="SetEventFilter"/>, this function does not
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_SetEventEnabled"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="PushEvent"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_RegisterEvents"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <seealso cref="WaitEventTimeout"/>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe nint GetWindowFromEvent(scoped ref readonly Event @event)
    {
        fixed (void* evPtr = &@event) return GetWindowFromEvent((nint)evPtr);
    }

    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetWindowFromEvent"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static partial nint GetWindowFromEvent(nint @event);

    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetEventDescription"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static partial int SDL_GetEventDescription(nint @event, nint buf, int buflen);
    ...
    /// <para>
    /// The exact format of the string is not guaranteed; it is intended for logging purposes, to be read by a human, and not
    ...
    /// <c> buflen </c> is zero), and will be truncated if <c> buflen </c> is too small. The return code is the number of bytes
    /// needed for the complete string, not counting the NULL-terminator, whether the string was truncated or not. Unlike
    /// SDL_snprintf(), though, this function never returns -1.
    /// </para>
    /// </summary>
    ...
    public static unsafe int GetEventDescription(scoped ref readonly Event @event, scoped Span<char> buf)
    {
        Span<byte> utf8Bytes = stackalloc byte[buf.Length + 1];

        fixed (void* ev = &@event)
        fixed (void* bufBytes = utf8Bytes)
            return SDL_GetEventDescription((nint)ev, (nint)bufBytes, utf8Bytes.Length);
    }
}