...
public static partial class SDL
{
    ...
    /// automatically by <see cref="HIDEnumerate"/> and any of the HIDOpen_*() functions if it is needed. This function should be
    /// called at the beginning of execution however, if there is a chance of HIDAPI handles being opened by different threads
    /// simultaneously.
    /// </para>
    ...
    /// <para>
    /// Enumerating the HID devices is an expensive operation, so you can call this to see if there have been any system
    /// device changes since the last call to this function. A change in the counter returned doesn't necessarily mean that anything
    /// has changed, but you can call <see cref="HIDEnumerate"/> to get an updated device list.
    /// </para>
    /// <para>
    /// Calling this function for the first time may cause a thread or other system resource to be allocated to track device
    /// change notifications.
    /// </para>
    /// </summary>
    /// <returns>
    /// a change counter that is incremented with each potential device change, or 0 if device change detection isn't
    /// available.
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="HIDEnumerate"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_device_change_count"),
    ...
    /// <para>
    /// This function returns a linked list of all the HID devices attached to the system which match <c> vendorId </c> and
    /// <c> productId </c>. If <c> vendorId </c> is set to 0 then any vendor matches. If <c> productId </c> is set to 0 then any
    /// product matches. If <c> vendorId </c> and <c> productId </c> are both set to 0, then all HID devices will be returned.
    /// </para>
    /// <para>
    /// By default SDL will only enumerate controllers, to reduce risk of hanging or crashing on bad drivers, but
    /// <see cref="Hints.HIDAPIEnumerateOnlyControllers"/> can be set to "0" to enumerate all HID devices.
    /// </para>
    /// </summary>
    ...
    /// <returns>
    /// a pointer to a linked list of type <see cref="HIDDeviceInfo"/>, containing information about the HID devices
    /// attached to the system, or NULL in the case of failure. Free this linked list by calling <see cref="HIDFreeEnumeration"/>.
    /// </returns>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_enumerate"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial nint HIDEnumerate(ushort vendorId, ushort productId);
    ...
    /// </summary>
    /// <param name="devs"> pointer to a list of struct_device returned from <see cref="HIDEnumerate"/>. </param>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_free_enumeration"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <para> If <c> serialNumber </c> is <c> null </c>, the first device with the specified VID and PID is opened. </para>
    /// </summary>
    ...
    /// <param name="serialNumber"> the Serial Number of the device to open (Optionally <c> null </c>). </param>
    /// <returns>
    /// a pointer to a SDL_hid_device object on success or <c> null </c> on failure; call <see cref="GetError"/> for more
    /// information.
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_open"), UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]),
     MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial nint HIDOpen(ushort vendorId,
        ushort productId,
        [MarshalUsing(typeof(WCharStringMarshaller))] string? serialNumber);
    ...
    /// <para>
    /// The path name be determined by calling <see cref="HIDEnumerate"/>, or a platform-specific path name can be used (eg:
    ...
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_open_path"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial nint HIDOpenPath([MarshalAs(UnmanagedType.LPUTF8Str)] string path);
    ...
    /// <para>
    /// The first byte of <c> data </c> must contain the Report ID. For devices which only support a single report, this must
    /// be set to 0x0. The remaining bytes contain the report data. Since the Report ID is mandatory, calls to
    /// <see cref="HIDWrite"/> will always contain one more byte than the report contains. For example, if a hid report is 16 bytes
    /// long, 17 bytes must be passed to <see cref="HIDWrite"/>, the Report ID (or 0x0, for devices with a single report), followed
    /// by the report data (16 bytes). In this example, the length passed in would be 17.
    /// </para>
    /// <para>
    /// <see cref="HIDWrite"/> will send the data on the first OUT endpoint, if one exists. If it does not, it will send the
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_write"), UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]),
     MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial int HIDWrite(nint dev,
    ...
    /// <param name="length">
    /// the number of bytes to read. For devices with multiple reports, make sure to read an extra byte for
    /// the report number.
    /// </param>
    /// <param name="milliseconds"> timeout in milliseconds or -1 for blocking wait. </param>
    /// <returns>
    /// the actual number of bytes read and -1 on on failure; call <see cref="GetError"/> for more information. If no
    /// packet was available to be read within the timeout period, this function returns 0.
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_read_timeout"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial int HIDReadTimeout(nint dev,
        [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] out byte[] data,
        nuint length,
        int milliseconds);
    ...
    /// </param>
    /// <returns>
    /// the actual number of bytes read and -1 on failure; call <see cref="GetError"/> for more information. If no packet
    /// was available to be read and the handle is in non-blocking mode, this function returns 0.
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_read"), UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]),
     MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial int HIDRead(nint dev,
        [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] out byte[] data,
        nuint length);
    ...
    /// <para>
    /// In non-blocking mode calls to <see cref="HIDRead"/> will return immediately with a value of 0 if there is no data to
    /// be read. In blocking mode, <see cref="HIDRead"/> will wait (block) until there is data to read before returning.
    /// </para>
    ...
    /// <returns> 0 on success or a negative error code on failure; call <see cref="GetError"/> for more information. </returns>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_set_nonblocking"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <para>
    /// Feature reports are sent over the Control endpoint as a Set_Report transfer. The first byte of <c> data </c> must
    /// contain the Report ID. For devices which only support a single report, this must be set to 0x0. The remaining bytes contain
    /// the report data. Since the Report ID is mandatory, calls to <see cref="HIDSendFeatureReport"/> will always contain one more
    /// byte than the report contains. For example, if a hid report is 16 bytes long, 17 bytes must be passed to
    /// <see cref="HIDSendFeatureReport"/>: the Report ID (or 0x0, for devices which do not use numbered reports), followed by the
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_send_feature_report"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial int HIDSendFeatureReport(nint dev,
        [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] byte[] data,
        nuint length);
    ...
    /// <para>
    /// Set the first byte of <c> data </c> to the Report ID of the report to be read. Make sure to allow space for this
    /// extra byte in <c> data </c>. Upon return, the first byte will still contain the Report ID, and the report data will start in
    /// data[1].
    /// </para>
    /// </summary>
    /// <param name="dev"> a device handle returned from <see cref="HIDOpen"/>. </param>
    /// <param name="data">
    /// a buffer to put the read data into, including the Report ID. Set the first byte of <c> data </c> to the
    /// Report ID of the report to be read, or set it to zero if your device does not use numbered reports.
    /// </param>
    /// <param name="length">
    /// the number of bytes to read, including an extra byte for the report ID. The buffer can be longer than
    /// the actual report.
    /// </param>
    /// <returns>
    /// the number of bytes read plus one for the report ID (which is still in the first byte), or -1 on on failure; call
    /// <see cref="GetError"/> for more information.
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_get_feature_report"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial int HIDGetFeatureReport(nint dev,
        [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] out byte[] data,
        nuint length);
    ...
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_get_input_report"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial int HidGetInputReport(nint dev,
        [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] out byte[] data,
        nuint length);
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_get_manufacturer_string"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static partial int SDL_hid_get_manufacturer_string(nint dev, nint @string, nuint maxlen);
    ...
    /// <returns> 0 on success or a negative error code on failure; call <see cref="GetError"/> for more information. </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    public static int HIDGetManufacturerString(nint dev, out string @string, nuint maxlen)
    {
        // Allocate a buffer for maxlen characters
        var maxbytes = unchecked(WCharStringMarshaller.WCharSize * maxlen);
        var buf = Malloc(maxbytes);
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_get_product_string"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static partial int SDL_hid_get_product_string(nint dev, nint @string, nuint maxlen);
    ...
    /// <param name="maxlen"> the length of the buffer in multiples of wchar_t. </param>
    /// <returns> 0 on success or a negative error code on failure; call <see cref="GetError"/> for more information. </returns>
    public static int HIDGetProductString(nint dev, out string @string, nuint maxlen)
    {
        // Allocate a buffer for maxlen characters
        var maxbytes = unchecked(WCharStringMarshaller.WCharSize * maxlen);
        var buf = Malloc(maxbytes);
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_get_serial_number_string"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static partial int SDL_hid_get_serial_number_string(nint dev, nint @string, nuint maxlen);
    ...
    /// <returns> 0 on success or a negative error code on failure; call <see cref="GetError"/> for more information. </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    public static int HIDGetSerialNumberString(nint dev, out string @string, nuint maxlen)
    {
        // Allocate a buffer for maxlen characters
        var maxbytes = unchecked(WCharStringMarshaller.WCharSize * maxlen);
        var buf = Malloc(maxbytes);
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_get_indexed_string"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static partial int SDL_hid_get_indexed_string(nint dev, int stringIndex, nint @string, nuint maxlen);
    ...
    /// <returns> 0 on success or a negative error code on failure; call <see cref="GetError"/> for more information. </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    public static int HIDGetIndexedString(nint dev, int stringIndex, out string @string, nuint maxlen)
    {
        // Allocate a buffer for maxlen characters
        var maxbytes = unchecked(WCharStringMarshaller.WCharSize * maxlen);
        var buf = Malloc(maxbytes);
    ...
    /// <param name="dev"> a device handle returned from <see cref="HIDOpen"/>. </param>
    /// <returns>
    /// a pointer to the SDL_hid_device_info for this hid_device or <c> null </c> on failure; call <see cref="GetError"/>
    /// for more information. This struct is valid until the device is closed with <see cref="HIDClose"/>.
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_get_device_info"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_hid_get_report_descriptor"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial int HIDGetReportDescriptor(nint dev,
