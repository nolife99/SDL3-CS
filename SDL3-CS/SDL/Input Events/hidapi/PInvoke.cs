...
public static partial class SDL
{
    ...
    ///         automatically by <see cref="HIDEnumerate"/> and any of the HIDOpen_*() functions if it is needed. This function
    ///         should be called at the beginning of execution however, if there is a chance of HIDAPI handles being opened by
    ...
    ///         device changes since the last call to this function. A change in the counter returned doesn't necessarily mean that
    ///         anything has changed, but you can call <see cref="HIDEnumerate"/> to get an updated device list.
    ///     </para>
    ///     <para>
    ///         Calling this function for the first time may cause a thread or other system resource to be allocated to track
    ...
    ///     <para>
    ///         This function returns a linked list of all the HID devices attached to the system which match <c> vendorId </c>
    ///         and <c> productId </c>. If <c> vendorId </c> is set to 0 then any vendor matches. If <c> productId </c> is set to 0
    ///         then any product matches. If <c> vendorId </c> and <c> productId </c> are both set to 0, then all HID devices will
    ///         be returned.
    ///     </para>
    ///     <para>
    ...
    ///     attached to the system, or NULL in the case of failure. Free this linked list by calling
    ///     <see cref="HIDFreeEnumeration"/>.
    /// </returns>
    ...
    ///     <para>
    ///         The path name be determined by calling <see cref="HIDEnumerate"/>, or a platform-specific path name can be used
    ...
    ///     <para>
    ///         The first byte of <c> data </c> must contain the Report ID. For devices which only support a single report, this
    ///         must be set to 0x0. The remaining bytes contain the report data. Since the Report ID is mandatory, calls to
    ///         <see cref="HIDWrite"/> will always contain one more byte than the report contains. For example, if a hid report is
    ///         16 bytes long, 17 bytes must be passed to <see cref="HIDWrite"/>, the Report ID (or 0x0, for devices with a single
    ///         report), followed by the report data (16 bytes). In this example, the length passed in would be 17.
    ///     </para>
    ///     <para>
    ///         <see cref="HIDWrite"/> will send the data on the first OUT endpoint, if one exists. If it does not, it will send
    ///         the data through the Control Endpoint (Endpoint 0).
    ///     </para>
    /// </summary>
    ...
    ///     <para>
    ///         In non-blocking mode calls to <see cref="HIDRead"/> will return immediately with a value of 0 if there is no data
    ///         to be read. In blocking mode, <see cref="HIDRead"/> will wait (block) until there is data to read before returning.
    ///     </para>
    ...
    ///         contain the Report ID. For devices which only support a single report, this must be set to 0x0. The remaining bytes
    ///         contain the report data. Since the Report ID is mandatory, calls to <see cref="HIDSendFeatureReport"/> will always
    ///         contain one more byte than the report contains. For example, if a hid report is 16 bytes long, 17 bytes must be
    ///         passed to <see cref="HIDSendFeatureReport"/>: the Report ID (or 0x0, for devices which do not use numbered reports),
    ...
    ///         extra byte in <c> data </c>. Upon return, the first byte will still contain the Report ID, and the report data will
    ///         start in data[1].
    ///     </para>
    /// </summary>
    /// <param name="dev"> a device handle returned from <see cref="HIDOpen"/>. </param>
