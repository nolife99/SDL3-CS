...
public static partial class SDL
{
 ...
 [LibraryImport(SDLLibrary, EntryPoint = "SDL_HasMouse"), UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]),
  MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetMouseNameForID"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static partial nint SDL_GetMouseNameForID(uint instanceId);
    ...
    /// <threadsafety> This function should only be called on the main thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetMouseFocus"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetMouseState"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <seealso cref="GetGlobalMouseState"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetGlobalMouseState"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <seealso cref="GetMouseState"/>
    /// <seealso cref="GetGlobalMouseState"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetRelativeMouseState"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_WarpMouseInWindow"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_WarpMouseGlobal"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    [return: MarshalAs(UnmanagedType.I1)]
    ...
    ///         constrained to the window, and SDL will report continuous relative mouse motion even if the mouse is at the edge of
    ///         the window.
    ///     </para>
    ///     <para>
    ///         If you'd like to keep the mouse position fixed while in relative mode you can use
    ///         <see cref="SetWindowMouseRect(IntPtr, in Rect)"/>. If you'd like the cursor to be at a specific location when
    ///         relative mode ends, you should use <see cref="WarpMouseInWindow"/> before disabling relative mode.
    ///     </para>
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_SetWindowRelativeMouseMode"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    [return: MarshalAs(UnmanagedType.I1)]
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetWindowRelativeMouseMode"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    [return: MarshalAs(UnmanagedType.I1)]
    ...
    ///     <para>
    ///         Capturing enables your app to obtain mouse events globally, instead of just within your window. Not all video
    ///         targets support this function. When capturing is enabled, the current window will get all mouse events, but unlike
    ///         relative mode, no change is made to the cursor and it is not restrained to your window.
    ///     </para>
    ///     <para>
    ///         This function may also deny mouse input to other windows--both those in your application and others on the
    ///         system--so you should use this function sparingly, and in small bursts. For example, you might want to track the
    ///         mouse while the user is dragging something, until the user releases a mouse button. It is not recommended that you
    ///         capture the mouse for long periods of time, such as the entire time your app is running. For that, you should
    ///         probably use <see cref="SetWindowRelativeMouseMode"/> or <see cref="SetWindowMouseGrab"/>, depending on your goals.
    ///     </para>
    ///     <para>
    ///         While captured, mouse events still report coordinates relative to the current (foreground) window, but those
    ///         coordinates may be outside the bounds of the window (including negative values). Capturing is only allowed for the
    ///         foreground window. If the window loses focus while capturing, the capture will be disabled automatically.
    ///     </para>
    ///     <para> While capturing is enabled, the current window will have the <see cref="WindowFlags.MouseCapture"/> flag set. </para>
    ///     <para>
    ///         Please note that SDL will attempt to "auto capture" the mouse while the user is pressing a button; this is to try
    ///         and make mouse behavior more consistent between platforms, and deal with the common case of a user dragging the
    ///         mouse outside of the window. This means that if you are calling <see cref="CaptureMouse"/> only to deal with this
    ///         situation, you do not have to (although it is safe to do so). If this causes problems for your app, you can disable
    ///         auto capture by setting the <see cref="Hints.MouseAutoCapture"/> hint to zero.
    ///     </para>
    /// </summary>
    ...
    ///     <list type="bullet">
    ///         <item> data=0, mask=1: white </item> <item> data=1, mask=1: black </item> <item> data=0, mask=0: transparent </item>
    ...
    ///         <see cref="CreateColorCursor"/>. Alternately, you can hide the cursor and draw your own as part of your game's
    ///         rendering, but it will be bound to the framerate.
    ///     </para>
    ///     <para>
    ///         Also, <see cref="CreateSystemCursor"/> is available, which provides several readily-available system cursors to
    ///         pick from.
    ///     </para>
    /// </summary>
    ...
    /// </param>
    /// <returns>
    ///     a new cursor with the specified parameters on success or <c> null </c> on failure; call <see cref="GetError"/> for
    ///     more information.
    /// </returns>
    /// <threadsafety> This function should only be called on the main thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_CreateCursor"), UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]),
     MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial nint CreateCursor(ReadOnlySpan<byte> data,
    ...
    ///     <para>
    ///         If this function is passed a surface with alternate representations added with
    ///         <see cref="AddSurfaceAlternateImage"/>, the surface will be interpreted as the content to be used for 100% display
    ///         scale, and the alternate representations will be used for high DPI situations. For example, if the original surface
    ///         is 32x32, then on a 2x macOS display or 200% display scale on Windows, a 64x64 version of the image will be used, if
    ///         available. If a matching version of the image isn't available, the closest larger size image will be downscaled to
    ///         the appropriate size and be used instead, if available. Otherwise, the closest smaller image will be upscaled and be
    ///         used instead.
    ///     </para>
    /// </summary>
    ...
    /// <seealso cref="DestroyCursor"/>
    /// <seealso cref="SetCursor"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_CreateColorCursor"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="DestroyCursor"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_CreateSystemCursor"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    ///     <para>
    ///         This function sets the currently active cursor to the specified one. If the cursor is currently visible, the
    ///         change will be immediately represented on the display. SetCursor(IntPtr.Zero) can be used to force cursor redraw, if
    ...
    ///     <para>
    ///         This function returns a pointer to the current cursor which is owned by the library. It is not necessary to free
    ///         the cursor with <see cref="DestroyCursor"/>.
    ///     </para>
    /// </summary>
    ...
    ///     <para> You do not have to call <see cref="DestroyCursor"/> on the return value, but it is safe to do so. </para>
    /// </summary>
    /// <returns> the default cursor on success or <c> null </c> on failuree; call <see cref="GetError"/> for more information. </returns>
    /// <threadsafety> This function should only be called on the main thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetDefaultCursor"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    ///     <para>
    ///         Use this function to free cursor resources created with <see cref="CreateCursor"/>,
    ///         <see cref="CreateColorCursor"/> or <see cref="CreateSystemCursor"/>.
    ///     </para>
    /// </summary>
    ...
    /// <seealso cref="CreateCursor"/>
    /// <seealso cref="CreateSystemCursor"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_DestroyCursor"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <seealso cref="HideCursor"/>
    /// <seealso cref="ShowCursor"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_CursorVisible"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    [return: MarshalAs(UnmanagedType.I1)]
