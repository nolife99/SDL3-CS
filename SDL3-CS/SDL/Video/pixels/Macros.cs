...
public static partial class SDL
{
	...
	/// <param name="order">
	///     the order of the new format, probably a <see cref="BitmapOrder"/>, <see cref="PackedOrder"/>, or
	///     <see cref="ArrayOrder"/> value.
	/// </param>
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static PixelFormat DefinePixelFormat(PixelType type, byte order, PackedLayout layout, byte bits, byte bytes)
        => (PixelFormat)(1 << 28 | (byte)type << 24 | order << 20 | (byte)layout << 16 | bits << 8 | bytes);

	/// <summary>
	///     <para> A macro to retrieve the flags of an <see cref="PixelFormat"/>. </para>
	///     <para>
	///         This macro is generally not needed directly by an app, which should use specific tests, like
	///         <see cref="IsPixelFormatFourCC"/>, instead.
	///     </para>
	/// </summary>
	...
	///     <para>
	///         This is usually a value from the <see cref="BitmapOrder"/>, <see cref="PackedOrder"/>, or
	///         <see cref="ArrayOrder"/> enumerations, depending on the format type.
	///     </para>
	/// </summary>
	/// <param name="x"> an <see cref="PixelFormat"/> to check. </param>
	...
	///     <para>
	///         This is usually a value from the <see cref="PackedLayout"/> enumeration, or zero if a layout doesn't make sense
	///         for the format type.
	///     </para>
	/// </summary>
	/// <param name="x"> an <see cref="PixelFormat"/> to check. </param>
	...
	[Macro]
    public static uint BytesPerPixel(PixelFormat x)
        => IsPixelFormatFourCC(x) ?
            x == PixelFormat.YUY2 || x == PixelFormat.UYVY || x == PixelFormat.YVYU || x == PixelFormat.P010 ? 2u : 1u :
            (uint)x >> 0 & 0xFF;
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static bool IsPixelFormatIndexed(PixelFormat x)
        => !IsPixelFormatFourCC(x) && (GetPixelType(x) == PixelType.Index1 || GetPixelType(x) == PixelType.Index2 ||
            GetPixelType(x) == PixelType.Index4 || GetPixelType(x) == PixelType.Index8);
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static bool IsPixelFormatPacked(PixelFormat x)
        => !IsPixelFormatFourCC(x) && (GetPixelType(x) == PixelType.Packed8 || GetPixelType(x) == PixelType.Packed16 ||
            GetPixelType(x) == PixelType.Packed32);
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static bool IsPixelFormatArray(PixelFormat x)
        => !IsPixelFormatFourCC(x) && (GetPixelType(x) == PixelType.ArrayU8 || GetPixelType(x) == PixelType.ArrayU16 ||
            GetPixelType(x) == PixelType.ArrayU32 || GetPixelType(x) == PixelType.ArrayF16 ||
            GetPixelType(x) == PixelType.ArrayF32);
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static bool IsPixelFormat10Bit(PixelFormat x)
        => !IsPixelFormatFourCC(x) &&
            (GetPixelType(x) == PixelType.Packed32 || PixelLayout(x) == PackedLayout.Layout2101010);
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static bool IsPixelFormatFloat(PixelFormat x)
        => !IsPixelFormatFourCC(x) && (GetPixelType(x) == PixelType.ArrayF16 || GetPixelType(x) == PixelType.ArrayF32);
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static bool IsPixelFormatAlpha(PixelFormat x)
        => IsPixelFormatPacked(x) && (PixelOrder(x) == (uint)PackedOrder.ARGB ||
            PixelOrder(x) == (uint)PackedOrder.RGBA || PixelOrder(x) == (uint)PackedOrder.ABGR ||
            PixelOrder(x) == (uint)PackedOrder.BGRA);
	...
	/// <param name="transfer">
	///     the transfer characteristics of the new format, probably an <see cref="TransferCharacteristics"/>
	///     value.
	/// </param>
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static Colorspace DefineColorspace(ColorType type,
        ColorRange range,
        ColorPrimaries primaries,
        TransferCharacteristics transfer,
        MatrixCoefficients matrix,
        ChromaLocation chroma)
        => (Colorspace)((byte)type << 28 | (byte)range << 24 | (byte)chroma << 20 | (byte)primaries << 10 |
            (byte)transfer << 5 | (byte)matrix);
	...
	/// <summary> A macro to retrieve the primaries of an <see cref="Colorspace"/>. </summary>
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static TransferCharacteristics ColorspaceTransfer(Colorspace cspace)
        => (TransferCharacteristics)((uint)cspace >> 5 & 0x1F);
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static bool IsColorspaceMatrixBT601(Colorspace cspace)
        => ColorspaceMatrix(cspace) == MatrixCoefficients.BT601 ||
            ColorspaceMatrix(cspace) == MatrixCoefficients.BT470BG;

	/// <summary> A macro to determine if an <see cref="Colorspace"/> uses BT709 matrix coefficients. </summary>
	/// <param name="cspace"> an <see cref="Colorspace"/> to check. </param>
	/// <returns> true if BT709, false otherwise. </returns>
	/// <threadsafety> It is safe to call this macro from any thread. </threadsafety>
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static bool IsColorspaceMatrixBT709(Colorspace cspace)
        => ColorspaceMatrix(cspace) == MatrixCoefficients.BT709;

	/// <summary>
	///     <para> A macro to determine if an <c> Colorspace </c> uses BT2020_NCL matrix coefficients. </para>
	/// </summary>
	/// <param name="cspace"> an <see cref="Colorspace"/> to check. </param>
	...
	/// <since> This macro is available since SDL 3.2.0 </since>
	[Macro]
    public static bool IsColorspaceMatrixBT2020NCL(Colorspace cspace)
        => ColorspaceMatrix(cspace) == MatrixCoefficients.BT2020NCL;

	/// <summary> A macro to determine if an <see cref="Colorspace"/> has a limited range. </summary>
	/// <param name="cspace"> an <see cref="Colorspace"/> to check. </param>
