...
{
    /// <summary>
    ///     <para> Pixel format. </para> <para> SDL's pixel formats have the following naming convention: </para>
    ///     <list type="bullet">
    ///         <item>
    ///             Names with a list of components and a single bit count, such as RGB24 and ABGR32, define a
    ///             platform-independent encoding into bytes in the order specified. For example, in RGB24 data, each pixel is
    ///             encoded in 3 bytes (red, green, blue) in that order, and in ABGR32 data, each pixel is encoded in 4 bytes
    ///             (alpha, blue, green, red) in that order. Use these names if the property of a format that is important to you is
    ///             the order of the bytes in memory or on disk.
    ///         </item>
    ///         <item>
    ///             Names with a bit count per component, such as ARGB8888 and XRGB1555, are "packed" into an appropriately-sized
    ///             integer in the platform's native endianness. For example, ARGB8888 is a sequence of 32-bit integers; in each
    ///             integer, the most significant bits are alpha, and the least significant bits are blue. On a little-endian CPU
    ///             such as x86, the least significant bits of each integer are arranged first in memory, but on a big-endian CPU
    ///             such as s390x, the most significant bits are arranged first. Use these names if the property of a format that is
    ///             important to you is the meaning of each bit position within a native-endianness integer.
    ///         </item>
    ///         <item>
    ///             In indexed formats such as INDEX4LSB, each pixel is represented by encoding an index into the palette into
    ///             the indicated number of bits, with multiple pixels packed into each byte if appropriate. In LSB formats, the
    ///             first (leftmost) pixel is stored in the least-significant bits of the byte; in MSB formats, it's stored in the
    ///             most-significant bits. INDEX8 does not need LSB/MSB variants, because each pixel exactly fills one byte.
    ///         </item>
    ///     </list>
    ///     <para>
    ///         The 32-bit byte-array encodings such as RGBA32 are aliases for the appropriate 8888 encoding for the current
    ///         platform. For example, RGBA32 is an alias for ABGR8888 on little-endian CPUs like x86, or an alias for RGBA8888 on
    ///         big-endian CPUs.
    ///     </para>
    /// </summary>
    ...
    public enum PixelFormat : uint
    {
        ...
        /// <summary>
        ///     <para> Planar mode: Y + V + U  (3 planes) </para> <code>DefinePixelFourCC('Y', 'V', '1', '2'),</code>
        /// </summary>
        YV12 = 0x32315659u,

        /// <summary>
        ///     <para> Planar mode: Y + U + V  (3 planes) </para> <code>DefinePixelFourCC('I', 'Y', 'U', 'V'),</code>
        /// </summary>
        IYUV = 0x56555949u,

        /// <summary>
        ///     <para> Packed mode: Y0+U0+Y1+V0 (1 plane) </para> <code>DefinePixelFourCC('Y', 'U', 'Y', '2'),</code>
        /// </summary>
        YUY2 = 0x32595559u,

        /// <summary>
        ///     <para> Packed mode: U0+Y0+V0+Y1 (1 plane) </para> <code>DefinePixelFourCC('U', 'Y', 'V', 'Y'),</code>
        /// </summary>
        UYVY = 0x59565955u,

        /// <summary>
        ///     <para> Packed mode: Y0+V0+Y1+U0 (1 plane) </para> <code>DefinePixelFourCC('Y', 'V', 'Y', 'U'),</code>
        /// </summary>
        YVYU = 0x55595659u,

        /// <summary>
        ///     <para> Planar mode: Y + U/V interleaved  (2 planes) </para> <code>DefinePixelFourCC('N', 'V', '1', '2'),</code>
        /// </summary>
        NV12 = 0x3231564eu,

        /// <summary>
        ///     <para> Planar mode: Y + V/U interleaved  (2 planes) </para> <code>DefinePixelFourCC('N', 'V', '2', '1'),</code>
        /// </summary>
        NV21 = 0x3132564eu,

        /// <summary>
        ///     <para> Planar mode: Y + U/V interleaved  (2 planes) </para> <code>DefinePixelFourCC('P', '0', '1', '0'),</code>
        /// </summary>
        P010 = 0x30313050u,

        /// <summary>
        ///     <para> Android video texture format </para> <code>DefinePixelFourCC('O', 'E', 'S', ' ')</code>
        /// </summary>
        ExternalOES = 0x2053454fu,

        /// <summary>
        ///     <para> Motion JPEG </para> <code>DefinePixelFourCC('M', 'J', 'P', 'G')</code>
        /// </summary>
        MJPG = 0x47504a4du
    }
}