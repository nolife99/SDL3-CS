...
public static partial class SDL
{
    ...
    /// <summary> Convert an <see cref="Rect"/> to <see cref="FRect"/> </summary>
    ...
    public static void RectToFRect(in Rect rect, out FRect frect)
        => frect = new() { X = rect.X, Y = rect.Y, W = rect.W, H = rect.H };
    ...
    ///     <para>
    ///         A point is considered part of a rectangle if both <c> p </c> and <c> r </c> are not <c> null </c>, and <c> p </c>
    ///         's x and y coordinates are >= to the rectangle's top left corner, and &lt; the rectangle's x+w and y+h. So a 1x1
    ///         rectangle considers point (0,0) as "inside" and (0,1) as not.
    ///     </para>
    ///     <para>
    ///         Note that this is a forced-inline function in a header, and not a public API function available in the SDL
    ///         library (which is to say, the code is embedded in the calling program and the linker and dynamic loader will not be
    ///         able to find this function inside SDL itself).
    ///     </para>
    /// </summary>
    ...
    public static bool PointInRect(in Point? p, in Rect? r)
    {
        ...
        return p.Value.X >= r.Value.X && p.Value.X < r.Value.X + r.Value.W && p.Value.Y >= r.Value.Y &&
    ...
    ///     <para>
    ///         A rectangle is considered "empty" for this function if <c> r </c> is <c> null </c>, or if <c> r </c>'s width
    ///         and/or height are &lt;= 0.
    ///     </para>
    ///     <para>
    ///         Note that this is a forced-inline function in a header, and not a public API function available in the SDL
    ...
    public static bool RectEmpty(in Rect? r) => r is not { W: > 0 } || r.Value.H <= 0;
    ...
    ///     <para> Rectangles are considered equal if both are not <c> null </c> and each of their x, y, width and height match. </para>
    ///     <para>
    ///         Note that this is a forced-inline function in a header, and not a public API function available in the SDL
    ///         library (which is to say, the code is embedded in the calling program and the linker and dynamic loader will not be
    ///         able to find this function inside SDL itself).
    ...
    public static bool RectsEqual(in Rect? a, in Rect? b)
        => a.HasValue && b.HasValue && a.Value.X == b.Value.X && a.Value.Y == b.Value.Y && a.Value.W == b.Value.W &&
            a.Value.H == b.Value.H;
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_HasRectIntersection"),
    ...
    /// <param name="b"> an <see cref="Rect"/> structure representing the second rectangle. </param>
    /// <param name="result">
    ///     an <see cref="Rect"/> structure filled in with the intersection of rectangles <c> a </c> and
    ///     <c> b </c>.
    /// </param>
    /// <returns> <c> true </c> if there is an intersection, <c> false </c> otherwise. </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="HasRectIntersection"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetRectIntersection"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    ...
    /// <param name="a"> an <see cref="Rect"/> structure representing the first rectangle. </param>
    /// <param name="b"> an <see cref="Rect"/> structure representing the second rectangle. </param>
    /// <param name="result"> an <see cref="Rect"/> structure filled in with the union of rectangles <c> A </c> and <c> B </c>. </param>
    ...
    ///     <para> If <c> clip </c> is not <c> null </c> then only points inside of the clipping rectangle are considered. </para>
    /// </summary>
    /// <param name="points"> an array of <see cref="Point"/> structures representing points to be enclosed. </param>
    ...
    /// <param name="result"> an <see cref="Rect"/> structure filled in with the minimal enclosing rectangle. </param>
    /// <returns>
    ///     <c> true </c> if any points were enclosed or <c> false </c> if all the points were outside of the clipping
    ///     rectangle.
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetRectEnclosingPoints"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool GetRectEnclosingPoints(
        [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 1)] Point[] points,
        int count,
        nint clip,
        out Rect result);
    ...
    ///         rectangle or that does not intersect will remain unchanged. A line segment that crosses the rectangle at either or
    ///         both ends will be clipped to the boundary of the rectangle and the new coordinates saved in <c> X1 </c>, <c> Y1 </c>
    ///         , <c> X2 </c>, and/or <c> Y2 </c> as necessary.
    ///     </para>
    /// </summary>
    ...
    /// <returns> <c> true </c> if there is an intersection, <c> false </c> otherwise. </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetRectAndLineIntersection"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    ...
    ///         A point is considered part of a rectangle if both <c> p </c> and <c> r </c> are not <c> null </c>, and <c> p </c>
    ///         's x and y coordinates are >= to the rectangle's top left corner, and &lt;= the rectangle's x+w and y+h. So a 1x1
    ///         rectangle considers point (0,0) and (0,1) as "inside" and (0,2) as not.
    ///     </para>
    ///     <para>
    ///         Note that this is a forced-inline function in a header, and not a public API function available in the SDL
    ///         library (which is to say, the code is embedded in the calling program and the linker and dynamic loader will not be
    ...
    public static bool PointInRectFloat(in FPoint? p, in FRect? r)
        => p.HasValue && r.HasValue && p.Value.X >= r.Value.X && p.Value.X < r.Value.X + r.Value.W &&
    ...
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    public static bool RectEmptyFloat(in FRect? r) => !r.HasValue || r.Value.W <= 0.0f || r.Value.H <= 0.0f;
    ...
    ///     <para>
    ///         Rectangles are considered equal if both are not <c> null </c> and each of their x, y, width and height are within
    ///         <c> epsilon </c> of each other. If you don't know what value to use for <c> epsilon </c>, you should call the
    ...
    public static bool RectsEqualEpsilon(in FRect? a, in FRect? b, in float epsilon)
    {
        ...
        return Math.Abs(rectA.X - rectB.X) <= epsilon && Math.Abs(rectA.Y - rectB.Y) <= epsilon &&
            Math.Abs(rectA.W - rectB.W) <= epsilon && Math.Abs(rectA.H - rectB.H) <= epsilon;
    }
    ...
    ///     <para>
    ///         Rectangles are considered equal if both are not <c> null </c> and each of their x, y, width and height are within
    ///         <see cref="FloatEpsilon"/> of each other. This is often a reasonable way to compare two floating point rectangles
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="GetRectIntersection"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_HasRectIntersectionFloat"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    ...
    /// <param name="b"> an <see cref="FRect"/> structure representing the second rectangle. </param>
    /// <param name="result">
    ///     an <see cref="FRect"/> structure filled in with the intersection of rectangles <c> a </c> and
    ///     <c> b </c>.
    /// </param>
    /// <returns> <c> true </c> if there is an intersection, <c> false </c> otherwise. </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="HasRectIntersectionFloat"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetRectIntersectionFloat"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    ...
    /// <param name="a"> an <see cref="FRect"/> structure representing the first rectangle. </param>
    /// <param name="b"> an <see cref="FRect"/> structure representing the second rectangle. </param>
    /// <param name="result"> an <see cref="FRect"/> structure filled in with the union of rectangles <c> A </c> and <c> B </c>. </param>
    /// <returns> <c> true </c> on success or <c> false </c> on failure; call <see cref="GetError"/> for more information. </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetRectUnionFloat"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    ...
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetRectEnclosingPointsFloat"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool GetRectEnclosingPointsFloat(
        [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 1)] FPoint[] points,
        int count,
        nint clip,
        out FRect result);
    ...
    /// <returns> <c> true </c> if there is an intersection, <c> false </c> otherwise. </returns>
    /// <since> </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetRectAndLineIntersectionFloat"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool GetRectAndLineIntersectionFloat(in FRect rect,
        ref float x1,
        ref float y1,
        ref float x2,
        ref float y2);
