...
///     <para>
///         While you can set most OpenGL attributes normally, the attributes listed above must be known before SDL creates
///         the window that will be used with the OpenGL context. These attributes are set and read with
///         <see cref="SDL.GLSetAttribute"/> and <see cref="SDL.GLGetAttribute"/>.
///     </para>
///     <para>
///         In some cases, these attributes are minimum requests; the GL does not promise to give you exactly what you asked
///         for. It's possible to ask for a 16-bit depth buffer and get a 24-bit one instead, for example, or to ask for no
///         stencil buffer and still have one available. Context creation should fail if the GL can't provide your requested
///         attributes at a minimum, but you should check to see exactly what you got.
///     </para>
/// </summary>
public enum GLAttr
{
    ...
    /// <summary>
    ///     sets context the release behavior. See <see cref="GLСontextReleaseFlag"/>; defaults to
    ///     <see cref="GLСontextReleaseFlag.Flush"/>.
    /// </summary>
    ContextReleaseBehavior,

    /// <summary>
    ///     set context reset notification. See <see cref="GLContextResetNotification"/>; defaults to
    ///     <see cref="GLContextResetNotification.NoNotification"/>.
    /// </summary>
