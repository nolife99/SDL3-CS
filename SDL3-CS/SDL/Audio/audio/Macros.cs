...
public static partial class SDL
{
    ...
    ///     <para>
    ///         SDL does not support custom audio formats, so this macro is not of much use externally, but it can be
    ///         illustrative as to what the various bits of an <see cref="AudioFormat"/> mean.
    ///     </para>
    ...
    public static ushort DefineAudioFormat(bool signed, bool bigendian, bool flt, int size)
    {
        ...
        if (signed) format |= 1 << 15;

        if (bigendian) format |= 1 << 12;
