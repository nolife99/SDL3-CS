...
public static partial class SDL
{
    ...
    ///     <para>
    ///         The priority controls the behavior when setting a hint that already has a value. Hints will replace existing
    ///         hints of their priority and lower. Environment variables are considered to have override priority.
    ///     </para>
    /// </summary>
    ...
    ///     <para>
    ///         Hints will not be set if there is an existing override hint or environment variable that takes precedence. You
    ///         can use <see cref="SetHintWithPriority"/> to set the hint with override priority instead.
    ///     </para>
    /// </summary>
    /// <param name="name"> the hint to set. </param>
    /// <param name="value"> the value of the hint variable. </param>
    ...
    ///     <para>
    ///         This will reset all hints to the value of the associated environment variable, or <c> null </c> if the
    ///         environment isn't set. Callbacks will be called normally with this change.
    ///     </para>
    /// </summary>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    ///     is changed; if another thread might do so, the app should supply locks and/or make a copy of the string. Note that using
    ///     a hint callback instead is always thread-safe, as SDL holds a lock on the thread subsystem during the callback.
    /// </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="SetHint"/>
    /// <seealso cref="SetHintWithPriority"/>
