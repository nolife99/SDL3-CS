...
public static partial class SDL
{
    ...
    /// <para>
    /// <see cref="Init"/> simply forwards to calling <see cref="InitSubSystem"/>. Therefore, the two may be used
    /// interchangeably. Though for readability of your cod <see cref="InitSubSystem"/> might be preferred.
    /// </para>
    /// <para>
    /// The file I/O (for example: <see cref="IOFromFile"/>) and threading (CreateThread) subsystems are initialized by
    /// default. Message boxes (<see cref="ShowSimpleMessageBox"/>) also attempt to work without initializing the video subsystem,
    /// in hopes of being useful in showing an error dialog when <see cref="Init"/> fails. You must specifically initialize other
    ...
    /// <item>
    /// <see cref="InitFlags.Video"/>: video subsystem; automatically initializes the events subsystem, should be initialized
    /// on the main thread.
    /// </item>
    ...
    /// </list>
    /// <para>
    /// Subsystem initialization is ref-counted, you must call <see cref="QuitSubSystem"/> for each
    /// <see cref="InitSubSystem"/> to correctly shutdown a subsystem manually (or call <see cref="Quit"/> to force shutdown). If a
    /// subsystem is already loaded then this call will increase the ref-count and return.
    /// </para>
    /// <para>
    /// Consider reporting some basic metadata about your application before calling <see cref="Init"/>, using either
    /// <see cref="SetAppMetadata"/> or <see cref="SetAppMetadataProperty"/>.
    /// </para>
    /// </summary>
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_InitSubSystem"),
    ...
    /// <seealso cref="InitSubSystem"/>
    /// <seealso cref="Quit"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_QuitSubSystem"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <param name="flags"> any of the flags used by <see cref="Init"/>; see <see cref="Init"/> for details. </param>
    /// <returns>
    /// a mask of all initialized subsystems if <c> flags </c> is <c> 0 </c>, otherwise it returns the initialization
    ...
    /// <seealso cref="QuitSubSystem"/>. It is safe to call this function even in the case of errors in initialization.
    /// </para>
    /// <para>
    /// You can use this function with atexit() to ensure that it is run when your application is shutdown, but it is not
    /// wise to do this from a library or other dynamically loaded code.
    /// </para>
    /// </summary>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="Init"/>
    ...
    /// <para>
    /// On Apple platforms, the main thread is the thread that runs your program's main() entry point. On other platforms,
    /// the main thread is the one that calls Init(WindowFlags.Video), which should usually be the one that runs your program's
    /// main() entry point. If you are using the main callbacks, SDL_AppInit(), SDL_AppIterate(), and SDL_AppQuit() are all called
    /// on the main thread.
    /// </para>
    /// </summary>
    ...
    /// <para>
    /// If this is called on the main thread, the callback is executed immediately. If this is called on another thread, this
    /// callback is queued for execution on the main thread during event processing.
    /// </para>
    /// <para>
    /// Be careful of deadlocks when using this functionality. You should not have the main thread wait for the current
    /// thread while this function is being called with <c> waitComplete </c> true.
    /// </para>
    /// </summary>
    ...
    /// <para>
    /// There are several locations where SDL can make use of metadata (an "About" box in the macOS menu bar, the name of the
    /// app can be shown on some audio mixers, etc). Any piece of metadata can be left as <c> null </c>, if a specific detail
    /// doesn't make sense for the app.
    /// </para>
    /// <para>
    /// This function should be called as early as possible, before <see cref="Init"/>. Multiple calls to this function are
    /// allowed, but various state might not change once it has been set up with a previous call to this function.
    /// </para>
    /// <para> Passing a <c> null </c> removes any previous metadata. </para>
    /// <para>
    /// This is a simplified interface for the most important information. You can supply significantly more detailed
    /// metadata with <see cref="SetAppMetadataProperty"/>.
    /// </para>
    /// </summary>
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="SetAppMetadataProperty"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_SetAppMetadata"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool SetAppMetadata([MarshalAs(UnmanagedType.LPUTF8Str)] string appname,
    ...
    /// <para> You can optionally provide metadata about your app to SDL. This is not required, but strongly encouraged. </para>
    /// <para>
    /// There are several locations where SDL can make use of metadata (an <c> "About" </c> box in the macOS menu bar, the
    /// name of the app can be shown on some audio mixers, etc). Any piece of metadata can be left out, if a specific detail doesn't
    ...
    /// <list type="bullet">
    /// <item>
    /// <see cref="Props.AppMetadataNameString"/>`: The human-readable name of the application, like
    /// <c> "My Game 2: Bad Guy's Revenge!" </c>. This will show up anywhere the OS shows the name of the application separately
    /// from window titles, such as volume control applets, etc. This defaults to <c> "SDL Application" </c>.
    /// </item>
    /// <item>
    /// <see cref="Props.AppMetadataVersionString"/>: The version of the app that is running; there are no rules on format,
    /// so <c> "1.0.3beta2" </c> and <c> "April 22nd, 2024" </c> and a git hash are all valid options. This has no default.
    /// </item>
    /// <item>
    /// <see cref="Props.AppMetadataIdentifierString"/>: A unique string that identifies this app. This must be in
    /// reverse-domain format, like <c> "com.example.mygame2" </c>. This string is used by desktop compositors to identify and group
    /// windows together, as well as match applications with associated desktop settings and icons. If you plan to package your
    /// application in a container such as Flatpak, the app ID should match the name of your Flatpak container as well. This has no
    /// default.
    /// </item>
    /// <item>
    /// <see cref="Props.AppMetadataCreatorString"/>: The human-readable name of the creator/developer/maker of this app,
    /// like <c> "MojoWorkshop, LLC" </c>
    /// </item>
    /// <item>
    /// <see cref="Props.AppMetadataCopyrightString"/>: The human-readable copyright notice, like
    /// <c> "Copyright (c) 2024 MojoWorkshop, LLC" </c> or whatnot. Keep this to one line, don't paste a copy of a whole software
    /// license in here. This has no default.
    /// </item>
    /// <item>
    /// <see cref="Props.AppMetadataURLString"/>: A URL to the app on the web. Maybe a product page, or a storefront, or even
    /// a GitHub repository, for user's further information This has no default.
    /// </item>
    /// <item>
    /// <see cref="Props.AppMetadataTypeString"/>: The type of application this is. Currently this string can be
    /// <c> "game" </c> for a video game, <c> "mediaplayer" </c> for a media player, or generically <c> "application" </c> if
    /// nothing else applies. Future versions of SDL might add new types. This defaults to <c> "application" </c>.
    /// </item>
    /// </list>
    /// </summary>
    ...
    /// <seealso cref="SetAppMetadata"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_SetAppMetadataProperty"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool SetAppMetadataProperty([MarshalAs(UnmanagedType.LPUTF8Str)] string name,
    ...
    /// <returns>
    /// the current value of the metadata property, or the default if it is not set, <c> null </c> for properties with no
    /// default.
    /// </returns>
    /// <threadsafety>
    /// It is safe to call this function from any thread, although the string returned is not protected and could
    /// potentially be freed if you call <see cref="SetAppMetadataProperty"/> to set that property from another thread.
    /// </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="SetAppMetadata"/>
    /// <seealso cref="SetAppMetadataProperty"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetAppMetadataProperty"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    [return: MarshalAs(UnmanagedType.I1)]
