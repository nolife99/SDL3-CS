...
public static partial class SDL
{
    ...
    public static partial bool SetErrorV([MarshalAs(UnmanagedType.LPUTF8Str)] string fmt,
    ...
    private static partial nint SDL_GetError();
    ...
    ///     <para>
    ///         It is possible for multiple errors to occur before calling <see cref="GetError"/>. Only the last error is
    ///         returned.
    ///     </para>
    ///     <para>
    ///         The message is only applicable when an SDL function has signaled an error. You must check the return values of
    ///         SDL function calls to determine when to appropriately call <see cref="GetError"/>. You should <b> not </b> use the
    ///         results of <see cref="GetError"/> to decide if an error has occurred! Sometimes SDL will set an error string even
    ///         when reporting success.
    ///     </para>
    ///     <para>
    ///         SDL will <b> not </b> clear the error string for successful API calls. You <b> must </b> check return values for
    ///         failure cases before you can assume the error string applies.
    ///     </para>
    ///     <para>
    ///         Error strings are set per-thread, so an error set in a different thread will not interfere with the current
    ///         thread's operation.
    ///     </para>
    ///     <para>
    ///         The returned value is a thread-local string which will remain valid until the current thread's error string is
    ///         changed. The caller should make a copy if the value is needed after the next SDL API call.
    ///     </para>
    /// </summary>
    /// <returns>
    ///     a message with information about the specific error that occurred, or an empty string if there hasn't been an
    ///     error message set since the last call to <see cref="ClearError"/>.
    /// </returns>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="ClearError"/>
