...
public static partial class SDL
{
    ...
    private static partial nint SDL_GetBasePath();
    ...
    private static partial nint SDL_GetPrefPath([MarshalAs(UnmanagedType.LPUTF8Str)] string org,
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetUserFolder"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial nint SDL_GetUserFolder(Folder folder);
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GlobDirectory"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial nint SDL_GlobDirectory([MarshalAs(UnmanagedType.LPUTF8Str)] string path,
    ...
    ///     </para>
    ///     <para>
    ///         The returned array is always NULL-terminated, for your iterating convenience, but if `count` is non-NULL, on
    ///         return it will contain the number of items in the array, not counting the <c> null </c> terminator.
    ///     </para>
    /// </summary>
    /// <param name="path"> the path of the directory to enumerate. </param>
    ...
    /// <returns>
    ///     an array of strings on success or <c> null </c> on failure; call <see cref="GetError"/> for more information. This
    ///     is a single allocation that should be freed with <see cref="Free"/> when it is no longer needed.
    /// </returns>
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetCurrentDirectory"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial nint SDL_GetCurrentDirectory();
    ...
    ///     <para>
    ///         For systems without a concept of a current working directory, this will still attempt to provide something
    ///         reasonable.
    ///     </para>
    ///     <para>
    ///         SDL does not provide a means to _change_ the current working directory; for platforms without this concept, this
    ///         would cause surprises with file access outside of SDL.
    ///     </para>
    ///     <para>
    ///         The returned path is guaranteed to end with a path separator (<c> \\ </c> on Windows, <c> / </c> on most other
    ///         platforms).
    ///     </para>
    /// </summary>
    /// <returns>
    ///     a UTF-8 string of the current working directory in platform-dependent notation. <c> null </c> if there's a
    ///     problem. This should be freed with <see cref="Free"/> when it is no longer needed.
    /// </returns>
