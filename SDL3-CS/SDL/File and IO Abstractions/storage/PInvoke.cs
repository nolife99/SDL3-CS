...
public static partial class SDL
{
    ...
    public static partial nint OpenTitleStorage([MarshalAs(UnmanagedType.LPUTF8Str)] string? @override, uint props);
    ...
    ///     <para>
    ///         While title storage can generally be kept open throughout runtime, user storage should only be opened when the
    ///         client is ready to read/write files. This allows the backend to properly batch file operations and flush them when
    ...
    ///     <para>
    ///         Applications do not need to use this function unless they are providing their own SDL_Storage implementation. If
    ///         you just need an SDL_Storage, you should use the built-in implementations in SDL, like
    ///         <see cref="OpenTitleStorage"/> or <see cref="OpenUserStorage"/>.
    ///     </para>
    ...
    ///         spinwait on this call, as the backend may depend on a synchronous message loop. You might instead poll this game's
    ///         main loop while processing events and drawing a loading screen.
    ///     </para>
    /// </summary>
    ...
    ///     <para>
    ///         The value of <c> length </c> must match the length of the file exactly; call <see cref="GetStorageFileSize"/> to
    ///         get this value. This behavior may be relaxed in a future release.
    ///     </para>
    /// </summary>
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_ReadStorageFile"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool ReadStorageFile(nint storage,
        [MarshalAs(UnmanagedType.LPUTF8Str)] string path,
        nint destination,
        ulong length);
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_WriteStorageFile"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool WriteStorageFile(nint storage,
        [MarshalAs(UnmanagedType.LPUTF8Str)] string path,
        nint source,
        ulong length);
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_EnumerateStorageDirectory"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool EnumerateStorageDirectory(nint storage,
        [MarshalAs(UnmanagedType.LPUTF8Str)] string path,
    ...
    ///         '*' (match everything) and '?' (match one character). If pattern is <c> null </c>, no filtering is done and all
    ///         results are returned. Subdirectories are permitted, and are specified with a path separator of '/'. Wildcard
    ///         characters '*' and '?' never match a path separator.
    ///     </para>
    ///     <para>
    ///         <c> flags </c> may be set to <see cref="GlobFlags.CaseInsensitive"/> to make the pattern matching
    ///         case-insensitive.
    ///     </para>
    ///     <para>
