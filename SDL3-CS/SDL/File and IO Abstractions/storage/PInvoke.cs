...
public static partial class SDL
{
    ...
    /// <para>
    /// By default, <see cref="OpenTitleStorage"/> uses the generic storage implementation. When the path override is not
    /// provided, the generic implementation will use the output of <see cref="GetBasePath"/> as the base path.
    /// </para>
    /// </summary>
    ...
    /// <returns>
    /// a title storage container on success or <c> null </c> on failure; call <see cref="GetError"/> for more
    /// information.
    /// </returns>
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_OpenTitleStorage"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial nint OpenTitleStorage([MarshalAs(UnmanagedType.LPUTF8Str)] string? @override, PropertiesID props);
    ...
    /// <para>
    /// While title storage can generally be kept open throughout runtime, user storage should only be opened when the client
    /// is ready to read/write files. This allows the backend to properly batch file operations and flush them when the container
    /// has been closed; ensuring safe and optimal save I/O.
    /// </para>
    /// </summary>
    /// <param name="org"> the name of your organization. </param>
    /// <param name="app"> the name of your application. </param>
    /// <param name="props"> a property list that may contain backend-specific information. </param>
    /// <returns> a user storage container on success or <c> null </c> on failure; call <see cref="GetError"/> for more information. </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="CloseStorage"/>
    /// <seealso cref="GetStorageFileSize"/>
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_OpenUserStorage"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial nint OpenUserStorage([MarshalAs(UnmanagedType.LPUTF8Str)] string org,
    ...
    /// <para>
    /// This is provided for development and tools. Portable applications should use <see cref="OpenTitleStorage"/> for
    /// access to game data and <see cref="OpenUserStorage"/> for access to user data.
    /// </para>
    /// </summary>
    /// <param name="path"> the base path prepended to all storage paths, or <c> null </c> for no base path. </param>
    /// <returns>
    /// a filesystem storage container on success or <c> null </c> on failure; call <see cref="GetError"/> for more
    /// information.
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="CloseStorage"/>
    ...
    /// <seealso cref="ReadStorageFile"/>
    /// <seealso cref="WriteStorageFile"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_OpenFileStorage"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial nint OpenFileStorage([MarshalAs(UnmanagedType.LPUTF8Str)] string? path);
    ...
    /// <para>
    /// Applications do not need to use this function unless they are providing their own SDL_Storage implementation. If you
    /// just need an SDL_Storage, you should use the built-in implementations in SDL, like <see cref="OpenTitleStorage"/> or
    /// <see cref="OpenUserStorage"/>.
    /// </para>
    ...
    /// <see cref="InitInterface(ref StorageInterface)"/>.
    /// </param>
    /// <param name="userdata"> the pointer that will be passed to the interface functions. </param>
    /// <returns> a storage container on success or <c> null </c> on failure; call <see cref="GetError"/> for more information. </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="CloseStorage"/>
    /// <seealso cref="GetStorageFileSize"/>
    /// <seealso cref="GetStorageSpaceRemaining"/>
    ...
    public static extern nint OpenStorage(in StorageInterface iface, nint userdata);
    ...
    /// <returns>
    /// <c> true </c> if the container was freed with no errors, <c> false </c> otherwise; call <see cref="GetError"/> for
    /// more information. Even if the function returns an error, the container data will be freed; the error is only for
    ...
    public static partial bool CloseStorage(nint storage);
    ...
    /// <para>
    /// This function should be called in regular intervals until it returns true - however, it is not recommended to
    /// spinwait on this call, as the backend may depend on a synchronous message loop. You might instead poll this game's main loop
    /// while processing events and drawing a loading screen.
    /// </para>
    /// </summary>
    ...
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool StorageReady(nint storage);
    ...
    /// <returns>
    /// <c> true </c> if the file could be queried or <c> false </c> on failure; call <see cref="GetError"/> for more
    /// information.
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="ReadStorageFile"/>
    /// <seealso cref="StorageReady"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetStorageFileSize"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool GetStorageFileSize(nint storage,
        [MarshalAs(UnmanagedType.LPUTF8Str)] string path,
        out ulong length);

    [LibraryImport(SDLLibrary, EntryPoint = "SDL_ReadStorageFile"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    private static unsafe partial bool SDL_ReadStorageFile(nint storage, nint path, void* destination, ulong length);
    ...
    /// <summary>
    /// <para> Synchronously read a file from a storage container into <paramref name="destination"/>. </para>
    /// <para>
    /// The span's byte length (<c> destination.Length * sizeof(T) </c>) must match the length of the file exactly; call
    /// <see cref="GetStorageFileSize"/> to get this value. The span is pinned for the call.
    /// </para>
    /// </summary>
    ...
    /// <returns> <c> true </c> if the file was read or <c> false </c> on failure; call <see cref="GetError"/> for more information. </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="GetStorageFileSize"/>
    /// <seealso cref="StorageReady"/>
    /// <seealso cref="WriteStorageFile{T}(nint, ReadOnlySpan{char}, ReadOnlySpan{T})"/>
    public static unsafe bool ReadStorageFile<T>(nint storage, scoped ReadOnlySpan<char> path, Span<T> destination)
        where T : unmanaged
    {
        using var u = new Utf8(path, stackalloc byte[512]);
        fixed (byte* pPath = u.Bytes)
        fixed (T* ptr = destination)
            return SDL_ReadStorageFile(storage, (nint)pPath, ptr, (ulong)destination.Length * (ulong)sizeof(T));
    }

    [LibraryImport(SDLLibrary, EntryPoint = "SDL_WriteStorageFile"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    private static unsafe partial bool SDL_WriteStorageFile(nint storage, nint path, void* source, ulong length);
    ...
    /// <summary>
    /// <para> Synchronously write <paramref name="source"/> from client memory into a storage container. </para>
    /// <para> The span (pinned for the call) contributes <c> source.Length * sizeof(T) </c> bytes. </para>
    /// </summary>
    /// <typeparam name="T"> the unmanaged element type of the span. </typeparam>
    ...
    /// <returns>
    /// <c> true </c> if the file was written or <c> false </c> on failure; call <see cref="GetError"/> for more
    /// information.
    /// </returns>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="GetStorageSpaceRemaining"/>
    /// <seealso cref="ReadStorageFile{T}(nint, ReadOnlySpan{char}, Span{T})"/>
    /// <seealso cref="StorageReady"/>
    public static unsafe bool WriteStorageFile<T>(nint storage, scoped ReadOnlySpan<char> path, ReadOnlySpan<T> source)
        where T : unmanaged
    {
        using var u = new Utf8(path, stackalloc byte[512]);
        fixed (byte* pPath = u.Bytes)
        fixed (T* ptr = source)
            return SDL_WriteStorageFile(storage, (nint)pPath, ptr, (ulong)source.Length * (ulong)sizeof(T));
    }
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="StorageReady"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_CreateStorageDirectory"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool CreateStorageDirectory(nint storage, [MarshalAs(UnmanagedType.LPUTF8Str)] string path);
    ...
    /// </para>
    /// <para>
    /// If <c> path </c> is <c> null </c>, this is treated as a request to enumerate the root of the storage container's
    /// tree. An empty string also works for this.
    /// </para>
    /// </summary>
    /// <param name="storage"> a storage container. </param>
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="StorageReady"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_EnumerateStorageDirectory"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    private static unsafe partial bool SDL_EnumerateStorageDirectory(nint storage,
        byte* path,
    ...
    static unsafe bool EnumerateStorageDirectoryCore(nint storage,
        ...
        DirectoryEnumerator enumerator)
    {
        ...
        try
        {
            fixed (byte* pathPtr = utf8)
                return SDL_EnumerateStorageDirectory(storage,
                    pathPtr,
                    &DirectoryEnumeratorThunk,
    ...
    public static bool EnumerateStorageDirectory(nint storage,
        scoped ReadOnlySpan<char> path,
        EnumerateDirectoryCallback callback)
    {
        ArgumentNullException.ThrowIfNull(callback);
        return EnumerateStorageDirectoryCore(storage, path, PlainDirectoryEnumerator.Rent(callback));
    }
    ...
    /// </summary>
    public static bool EnumerateStorageDirectory<T>(nint storage,
        scoped ReadOnlySpan<char> path,
        EnumerateDirectoryCallback<T> callback,
        T state)
    {
        ArgumentNullException.ThrowIfNull(callback);
        return EnumerateStorageDirectoryCore(storage, path, StatefulDirectoryEnumerator<T>.Rent(callback, state));
    }
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="StorageReady"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_RemoveStoragePath"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool RemoveStoragePath(nint storage, [MarshalAs(UnmanagedType.LPUTF8Str)] string path);
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="StorageReady"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_RenameStoragePath"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool RenameStoragePath(nint storage,
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="StorageReady"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_CopyStorageFile"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool CopyStorageFile(nint storage,
        [MarshalAs(UnmanagedType.LPUTF8Str)] string oldpath,
        [MarshalAs(UnmanagedType.LPUTF8Str)] string newpath);
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="StorageReady"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetStoragePathInfo"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool GetStoragePathInfo(nint storage,
        [MarshalAs(UnmanagedType.LPUTF8Str)] string path,
        out PathInfo info);
    ...
    /// <seealso cref="StorageReady"/>
    /// <seealso cref="WriteStorageFile"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetStorageSpaceRemaining"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial ulong GetStorageSpaceRemaining(nint storage);

    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GlobStorageDirectory"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    private static partial nint SDL_GlobStorageDirectory(nint storage,
        [MarshalAs(UnmanagedType.LPUTF8Str)] string path,
    ...
    /// <para>
    /// Files are filtered out if they don't match the string in <c> pattern </c>, which may contain wildcard characters '*'
    /// (match everything) and '?' (match one character). If pattern is <c> null </c>, no filtering is done and all results are
    ...
    ///     </para>
    ///     <para>
    ///         The returned array is always NULL-terminated, for your iterating convenience, but if <c> count </c> is non-NULL,
    ///         on return it will contain the number of items in the array, not counting the <c> null </c> terminator.
    ///     </para>
    ///     <para>
    ///         If <c> path </c> is <c> null </c>, this is treated as a request to enumerate the root of the storage container's
    ///         tree. An empty string also works for this.
    ...
    /// <param name="storage"> a storage container. </param>
    /// <param name="path"> the path of the directory to enumerate, or <c> null </c> for the root. </param>
    /// <param name="pattern"> the pattern that files in the directory must match. Can be <c> null </c>. </param>
    /// <param name="flags"> <c> SDL_GLOB_* </c> bitflags that affect this search. </param>
    /// <param name="count"> on return, will be set to the number of items in the returned array. Can be <c> null </c>. </param>
    /// <returns>
    ///     an array of strings on success or <c> null </c> on failure; call <see cref="GetError"/> for more information. The
    ///     caller should pass the returned pointer to <see cref="Free"/> when done with it. This is a single allocation that should
    ///     be freed with <see cref="Free"/> when it is no longer needed.
    /// </returns>
    ...
    /// <since> This function is available since SDL 3.2.0 </since>
    public static string[]? GlobStorageDirectory(nint storage,
        string path,
        string? pattern,
        GlobFlags flags,
        out int count)
