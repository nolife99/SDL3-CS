...
public static partial class SDL
{
    ...
    /// <summary>
    /// <para> The prototype for an Apple iOS animation callback. </para> <para> This datatype is only useful on Apple iOS. </para>
    /// <para>
    /// After passing a function pointer of this type to <see cref="SetiOSAnimationCallback"/>, the system will call that
    /// function pointer at a regular interval.
    /// </para>
    /// <param name="userdata">
    /// what was passed as <c> callbackParam </c> to <see cref="SetiOSAnimationCallback"/> as
    /// <c> callbackParam </c>.
    /// </param>
    /// </summary>
    ...
    /// <summary> Callback that presents a response from a <see cref="RequestAndroidPermission"/> call. </summary>
    ...
    public delegate void RequestAndroidPermissionCallback(ReadOnlySpan<byte> permission, bool granted);
    ...
    /// <para>
    /// This callback may modify the message, and should return true if the message should continue to be processed, or false
    /// to prevent further processing.
    /// </para>
    /// <para>
    /// As this is processing a message directly from the Windows event loop, this callback should do the minimum required
    /// work and return quickly.
    /// </para>
    /// </summary>
    /// <param name="userdata"> the app-defined pointer provided to <see cref="SetWindowsMessageHook"/>. </param>
    ...
    /// <threadsafety> This may only be called (by SDL) from the thread handling the Windows event loop. </threadsafety>
    /// <since> This datatype is available since SDL 3.2.0 </since>
    ...
    /// <para>
    /// This callback may modify the event, and should return true if the event should continue to be processed, or false to
    /// prevent further processing.
    /// </para>
    /// <para>
    /// As this is processing an event directly from the X11 event loop, this callback should do the minimum required work
    /// and return quickly.
    /// </para>
    /// </summary>
    ...
    /// <returns> <c> true </c> to let event continue on, <c> false </c> to drop it. </returns>
    /// <threadsafety> This may only be called (by SDL) from the thread handling the X11 event loop. </threadsafety>
    /// <since> This datatype is available since SDL 3.2.0 </since>
