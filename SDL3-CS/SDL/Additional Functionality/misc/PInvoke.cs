...
public static unsafe partial class SDL
{
    ...
    ///         This will likely launch what makes sense to handle a specific URL's protocol (a web browser for <c> http:// </c>,
    ///         etc), but it might also be able to launch file managers for directories and other things.
    ///     </para>
    ///     <para>
    ///         What happens when you open a URL varies wildly as well: your game window may lose focus (and may or may not lose
    ///         focus if your game was fullscreen or grabbing input at the time). On mobile devices, your app will likely move to
    ///         the background or your process might be paused. Any given platform may or may not handle a given URL.
    ///     </para>
    ///     <para>
    ///         If this is unimplemented (or simply unavailable) for a platform, this will fail with an error. A successful
    ///         result does not mean the URL loaded, just that we launched _something_ to handle it (or at least believe we did).
    ///     </para>
    ...
    {
        var byteCount = Encoding.UTF8.GetByteCount(url) + 1;
        ...
        try
        {
            var written = Encoding.UTF8.GetBytes(url, buffer);
            buffer[written] = 0;

            fixed (byte* pBuffer = buffer) return OpenURLNative(pBuffer);
        }
        finally
        {
