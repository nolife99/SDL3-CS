...
public partial class SDL
{
    ...
    /// <para> The allocated memory returned by this function must be freed with <see cref="Free"/>. </para>
    /// <para> If <c> size </c> is 0, it will be set to 1. </para>
    /// <para> If you want to allocate memory aligned to a specific alignment, consider using <see cref="AlignedAlloc"/>. </para>
    /// <para>
    /// If the allocation is successful, the returned pointer is guaranteed to be aligned to either the *fundamental
    /// alignment* (`alignof(max_align_t)` in C11 and later) or `2 * sizeof(void *)`, whichever is smaller. Use SDL_aligned_alloc()
    /// if you need to allocate memory aligned to an alignment greater than this guarantee.
    /// </para>
    /// </summary>
    ...
    /// <para>
    /// If the allocation is successful, the returned pointer is guaranteed to be aligned to either the *fundamental
    /// alignment* (`alignof(max_align_t)` in C11 and later) or `2 * sizeof(void *)`, whichever is smaller.
    /// </para>
    /// </summary>
    ...
    /// <para> The memory returned by this function must be freed with <see cref="Free"/>. </para>
    /// <para>
    /// If <c> size </c> is 0, it will be set to 1. Note that this is unlike some other C runtime <c> realloc </c>
    /// implementations, which may treat <c> realloc(mem, 0) </c> the same way as <c> free(mem) </c>.
    /// </para>
    /// <para>
    /// If <c> mem </c> is <c> null </c>, the behavior of this function is equivalent to <see cref="Malloc"/>. Otherwise, the
    /// function can have one of three possible outcomes:
    /// </para>
    /// <list type="bullet">
    /// <item> If it returns the same pointer as <c> mem </c>, it means that <c> mem </c> was resized in place without freeing. </item>
    /// <item> If it returns a different non-NULL pointer, it means that <c> mem </c> was freed and cannot be dereferenced anymore. </item>
    /// <item>
    /// If it returns <c> null </c> (indicating failure), then <c> mem </c> will remain valid and must still be freed with
    /// <see cref="Free"/>.
    /// </item>
    /// </list>
    /// <para>
    /// If the allocation is successfully resized, the returned pointer is guaranteed to be aligned to either the
    /// *fundamental alignment* (`alignof(max_align_t)` in C11 and later) or `2 * sizeof(void *)`, whichever is smaller.
    /// </para>
    /// </summary>
    ...
    /// <returns> a pointer to the newly allocated memory, or <c> null </c> if allocation failed. </returns>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="Free"/>
    /// <seealso cref="Malloc"/>
    ...
    /// <para>
    /// This is what <see cref="Malloc"/> and friends will use by default, if there has been no call to
    /// <see cref="SetMemoryFunctions"/>. This is not necessarily using the C runtime's <c> malloc </c> functions behind the scenes!
    ...
    /// <param name="freeFunc"> filled with free function. </param>
    /// <threadsafety>
    /// This does not hold a lock, so do not call this in the unlikely event of a background thread calling
    /// <see cref="SetMemoryFunctions"/> simultaneously.
    /// </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    /// <para>
    /// It is not safe to call this function once any allocations have been made, as future calls to SDL_free will use the
    /// new allocator, even if they came from an SDL_malloc made with the old one!
    /// </para>
    /// <para>
    ...
    /// <threadsafety>
    /// It is safe to call this function from any thread, but one should not replace the memory functions once any
    ...
    /// <para> The memory returned by this function must be freed with <see cref="AlignedFree"/>, _not_ <see cref="Free"/>. </para>
    /// <para> If <c> alignment </c> is less than the size of <c> void * </c>, it will be increased to match that. </para>
    /// <para>
    /// The returned memory address will be a multiple of the alignment value, and the size of the memory allocated will be a
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_aligned_alloc"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetNumAllocations"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <see cref="SetEnvironmentVariable"/> and <see cref="UnsetEnvironmentVariable"/> if you want to modify this environment, or
    /// SDL_setenv_unsafe() or SDL_unsetenv_unsafe() if you want changes to persist in the C runtime environment after
    /// <see cref="Quit"/>.
    /// </para>
    /// </summary>
    /// <returns>
    /// a pointer to the environment for the process or <c> null </c> on failure; call <see cref="GetError"/> for more
    /// information.
    /// </returns>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetEnvironment"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary> Create a set of environment variables </summary>
    /// <param name="populated"> true to initialize it from the C runtime environment, false to create an empty environment. </param>
    /// <returns> a pointer to the new environment or <c> null </c> on failure; call <see cref="GetError"/> for more information. </returns>
    /// <threadsafety>
    /// If <c> populated </c> is false, it is safe to call this function from any thread, otherwise it is safe if no
    /// other threads are calling setenv() or unsetenv()
    /// </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="GetEnvironmentVariable"/>
    /// <seealso cref="GetEnvironmentVariables"/>
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_CreateEnvironment"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static partial nint CreateEnvironment([MarshalAs(UnmanagedType.I1)] bool populated);

    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetEnvironmentVariable"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static partial nint SDL_GetEnvironmentVariable(nint env, [MarshalAs(UnmanagedType.LPUTF8Str)] string name);
    ...
    /// <returns> a pointer to the value of the variable or <c> null </c> if it can't be found. </returns>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    /// <seealso cref="SetEnvironmentVariable"/>
    /// <seealso cref="UnsetEnvironmentVariable"/>
    public static string? GetEnvironmentVariable(nint env, string name)
    {
        var value = SDL_GetEnvironmentVariable(env, name);
        return value == nint.Zero ? null : Marshal.PtrToStringUTF8(value);
    }

    [LibraryImport(SDLLibrary, EntryPoint = "SDL_GetEnvironmentVariables"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static partial nint SDL_GetEnvironmentVariables(nint env);

    /// <code>extern SDL_DECLSPEC char ** SDLCALL SDL_GetEnvironmentVariables(SDL_Environment *env);</code>
    /// <summary> Get all variables in the environment. </summary>
    /// <param name="env"> the environment to query. </param>
    /// <returns>
    /// a <c> null </c> terminated array of pointers to environment variables in the form "variable=value" or
    /// <c> null </c> on failure; call <see cref="GetError"/> for more information. This is a single allocation that should be freed
    /// with <see cref="Free"/> when it is no longer needed.
    /// </returns>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="GetEnvironment"/>
    ...
    /// <seealso cref="SetEnvironmentVariable"/>
    /// <seealso cref="UnsetEnvironmentVariable"/>
    public static string[]? GetEnvironmentVariables(nint env)
    {
    ...
    /// <summary> Set the value of a variable in the environment. </summary>
    /// <param name="env"> the environment to modify. </param>
    /// <param name="name"> the name of the variable to set. </param>
    /// <param name="value"> the value of the variable to set. </param>
    /// <param name="overwrite">
    /// <c> true </c> to overwrite the variable if it exists, <c> false </c> to return success without
    /// setting the variable if it already exists.
    /// </param>
    /// <returns> <c> true </c> on success or <c> false </c> on failure; call <see cref="GetError"/> for more information. </returns>
    /// <threadsafety> It is safe to call this function from any thread. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    ...
    /// <seealso cref="GetEnvironmentVariables"/>
    /// <seealso cref="UnsetEnvironmentVariable"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_SetEnvironmentVariable"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool SetEnvironmentVariable(nint env,
    ...
    /// <summary> Clear a variable from the environment. </summary>
    /// <param name="env"> the environment to modify. </param>
    ...
    /// <seealso cref="SetEnvironmentVariable"/>
    /// <seealso cref="UnsetEnvironmentVariable"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_UnsetEnvironmentVariable"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    [return: MarshalAs(UnmanagedType.I1)]
    public static partial bool UnsetEnvironmentVariable(nint env, [MarshalAs(UnmanagedType.LPUTF8Str)] string name);

    /// <code>extern SDL_DECLSPEC void SDLCALL SDL_DestroyEnvironment(SDL_Environment *env);</code>
    /// <summary> Destroy a set of environment variables. </summary>
    /// <param name="env"> the environment to destroy. </param>
    /// <threadsafety> It is safe to call this function from any thread, as long as the environment is no longer in use. </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="CreateEnvironment"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_DestroyEnvironment"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <para> Reusing the seed number will cause Rand() to repeat the same stream of <c> random </c> numbers. </para>
    /// </summary>
    /// <param name="seed"> the value to use as a random number seed, or 0 to use <see cref="GetPerformanceCounter"/>. </param>
    /// <threadsafety> This should be called on the same thread that calls Rand*() </threadsafety>
    /// <since> This function is available since SDL 3.2.0 </since>
    /// <seealso cref="Rand"/>
    /// <seealso cref="RandBits"/>
    /// <seealso cref="RandF"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_srand"),
    ...
    /// <para>
    /// The method used is faster and of better quality than <c> rand() % n </c>. Odds are roughly 99.9% even for n = 1
    /// million. Evenness is better for smaller n, and much worse as n gets bigger.
    /// </para>
    /// <para> Example: to simulate a d6 use <c> Rand(6) + 1 </c> The +1 converts 0..5 to 1..6 </para>
    /// <para> If you want to generate a pseudo-random number in the full range of Sint32, you should use: (int)RandBits() </para>
    /// <para> If you want reproducible output, be sure to initialize with SDL_srand() first. </para>
    /// <para>
    /// There are no guarantees as to the quality of the random sequence produced, and this should not be used for security
    /// (cryptography, passwords) or where money is on the line (loot-boxes, casinos). There are many random number libraries
    /// available with different characteristics and you should pick one of those to meet any serious needs.
    /// </para>
    /// </summary>
    ...
    /// <para> If you want reproducible output, be sure to initialize with <see cref="SRand"/> first. </para>
    /// <para>
    /// There are no guarantees as to the quality of the random sequence produced, and this should not be used for security
    /// (cryptography, passwords) or where money is on the line (loot-boxes, casinos). There are many random number libraries
    /// available with different characteristics and you should pick one of those to meet any serious needs.
    ...
    /// million. Evenness is better for smaller n, and much worse as n gets bigger.
    /// </para>
    /// <para> Example: to simulate a d6 use <c> RandR(state, 6) + 1 </c> The +1 converts 0..5 to 1..6 </para>
    /// <para> If you want to generate a pseudo-random number in the full range of Sint32, you should use: (int)RandBitsR(state) </para>
    /// <para>
    /// There are no guarantees as to the quality of the random sequence produced, and this should not be used for security
    /// (cryptography, passwords) or where money is on the line (loot-boxes, casinos). There are many random number libraries
    /// available with different characteristics and you should pick one of those to meet any serious needs.
    /// </para>
    /// </summary>
    /// <param name="state"> a pointer to the current random number state, this may not be <c> null </c>. </param>
    /// <param name="n"> the number of possible outcomes. n must be positive. </param>
    /// <returns> a random value in the range of [0 .. n-1]. </returns>
    ...
    /// <seealso cref="RandFR"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_rand_r"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl), typeof(CallConvSuppressGCTransition)]),
     MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <seealso cref="RandR"/>
    /// <seealso cref="RandF"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_randf_r"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl), typeof(CallConvSuppressGCTransition)]),
     MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <seealso cref="RandR"/>
    /// <seealso cref="RandFR"/>
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_rand_bits_r"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl), typeof(CallConvSuppressGCTransition)]),
     MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <para> This function will set <c> len </c> bytes, pointed to by <c> dst </c>, to the value specified in <c> c </c>. </para>
    /// <para>
    /// Despite <c> c </c> being an <c> int </c> instead of a <c> char </c>, this only operates on bytes; <c> c </c> must be
