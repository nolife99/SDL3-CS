...
public partial class SDL
{
 ...
 ///         alignment* (`alignof(max_align_t)` in C11 and later) or `2 * sizeof(void *)`, whichever is smaller. Use
 ...
 ///     </para>
 ///     <para>
 ///         If <c> mem </c> is <c> null </c>, the behavior of this function is equivalent to <see cref="Malloc"/>. Otherwise,
 ///         the function can have one of three possible outcomes:
 ///     </para>
 ///     <list type="bullet">
 ///         <item>
 ///             If it returns the same pointer as <c> mem </c>, it means that <c> mem </c> was resized in place without
 ///             freeing.
 ///         </item>
 ///         <item>
 ///             If it returns a different non-NULL pointer, it means that <c> mem </c> was freed and cannot be dereferenced
 ///             anymore.
 ///         </item>
 ///         <item>
 ///             If it returns <c> null </c> (indicating failure), then <c> mem </c> will remain valid and must still be freed
 ///             with <see cref="Free"/>.
 ///         </item>
 ///     </list>
 ///     <para>
 ...
 ///         <see cref="SetMemoryFunctions"/>. This is not necessarily using the C runtime's <c> malloc </c> functions behind the
 ///         scenes! Different platforms and build configurations might do any number of unexpected things.
 ///     </para>
 /// </summary>
 ...
 ///     <para>
 ///         It is not safe to call this function once any allocations have been made, as future calls to SDL_free will use
 ///         the new allocator, even if they came from an SDL_malloc made with the old one!
 ///     </para>
 ///     <para>
 ...
 ///         Use <see cref="SetEnvironmentVariable"/> and <see cref="UnsetEnvironmentVariable"/> if you want to modify this
 ///         environment, or SDL_setenv_unsafe() or SDL_unsetenv_unsafe() if you want changes to persist in the C runtime
 ///         environment after <see cref="Quit"/>.
 ///     </para>
 /// </summary>
 /// <returns>
 ...
 [LibraryImport(SDLLibrary, EntryPoint = "SDL_CreateEnvironment"),
  UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)]), MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    ///     <c> null </c> on failure; call <see cref="GetError"/> for more information. This is a single allocation that should be
    ...
    ///     <para>
    ///         There are no guarantees as to the quality of the random sequence produced, and this should not be used for
    ///         security (cryptography, passwords) or where money is on the line (loot-boxes, casinos). There are many random number
    ...
    ///     <para>
    ///         Despite <c> c </c> being an <c> int </c> instead of a <c> char </c>, this only operates on bytes; <c> c </c> must
    ///         be a value between 0 and 255, inclusive.
    ///     </para>
    /// </summary>
