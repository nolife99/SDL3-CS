...
public static partial class SDL
{
    ...
    /// </list>
    /// <para>
    /// Note that this "function" is actually a macro that calls an internal function with two extra parameters not listed
    /// here; they are hidden through preprocessor macros and are needed to support various C runtimes at the point of the function
    /// call. Language bindings that aren't using the C headers will need to deal with this.
    /// </para>
    ...
    public static nint CreateThread(ThreadFunction fn, string name) => CreateThreadRuntime(fn, name);

    /// <code>extern SDL_DECLSPEC SDL_Thread * SDLCALL SDL_CreateThreadWithProperties(SDL_PropertiesID props);</code>
    /// <summary>
    /// <para> Create a new thread with with the specified properties. </para> <para> These are the supported properties: </para>
    /// <list type="bullet">
    /// <item>
    /// <see cref="Props.ThreadCreateEntryFunctionPointer"/>: an SDL_ThreadFunction value that will be called at the start of
    /// the new thread's life. Required.
    /// </item>
    /// <item>
    /// <see cref="Props.ThreadCreateNameString"/>: the name of the new thread, which might be available to debuggers.
    /// Optional, defaults to <c> null </c>.
    /// </item>
    /// <item>
    /// <see cref="Props.ThreadCreateUserdataPointer"/>: an arbitrary app-defined pointer, which is passed to the entry
    /// function on the new thread, as its only parameter. Optional, defaults to <c> null </c>.
    /// </item>
    /// <item>
    /// <see cref="Props.ThreadCreateStacksizeNumber"/>: the size, in bytes, of the new thread's stack. Optional, defaults to
    /// 0 (system-defined default).
    /// </item>
    /// </list>
    /// <para>
    ...
    /// Linux currently has 16, Visual C++ 6.0 has _nine_!), and possibly other arbitrary rules. You'll have to see what happens
    /// with your system's debugger. The name should be UTF-8 (but using the naming limits of C identifiers is a better bet). There
    /// are no requirements for thread naming conventions, so long as the string is null-terminated UTF-8, but these guidelines are
    ...
    /// contents will be available from <see cref="GetThreadName"/>.
    /// </para>
    /// <para>
    /// The size (in bytes) of the new stack can be specified with <see cref="Props.ThreadCreateStacksizeNumber"/>. Zero
    /// means "use the system default" which might be wildly different between platforms. x86 Linux generally defaults to eight
    /// megabytes, an embedded device might be a few kilobytes instead. You generally need to specify a stack that is a multiple of
    /// the system's page size (in many cases, this is 4 kilobytes, but check your system documentation).
    /// </para>
    /// <para>
    /// Note that this "function" is actually a macro that calls an internal function with two extra parameters not listed
    /// here; they are hidden through preprocessor macros and are needed to support various C runtimes at the point of the function
    ...
    /// </para>
    /// <para>
    /// The actual symbol in SDL is <see cref="CreateThreadWithPropertiesRuntime"/>, so there is no symbol clash, but trying
    /// to load an SDL shared library and look for <see cref="CreateThreadWithProperties"/> will fail.
    /// </para>
    /// <para> Usually, apps should just call this function the same way on every platform and let the macros hide the details. </para>
    /// </summary>
    ...
    /// <seealso cref="WaitThread"/>
    public static nint CreateThreadWithProperties(PropertiesID props) => CreateThreadWithPropertiesRuntime(props, null, null);
    ...
    private static unsafe partial nint SDL_CreateThreadRuntime(
        delegate* unmanaged[Cdecl]<nint, int> fn,
        [MarshalAs(UnmanagedType.LPUTF8Str)] string name,
        nint data,
        nint pfnBeginThread,
        nint pfnEndThread);

    /// <summary>Runs once on the new thread; the root is freed when the thread function returns.</summary>
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    static int ThreadFunctionThunk(nint userdata)
    {
        var handle = GCHandle.FromIntPtr(userdata);
        try
        {
            return ((ThreadFunction)handle.Target!)();
        }
    ...
    /// <summary>
    ///     The actual entry point for <see cref="CreateThread"/>. State is captured by closure; the
    ///     delegate is rooted until the thread function returns (or this call fails). The begin/end
    ///     runtime pointers are unused on modern platforms and passed as null.
    /// </summary>
    public static unsafe nint CreateThreadRuntime(ThreadFunction fn, string name)
    {
        ArgumentNullException.ThrowIfNull(fn);

        var handle = GCHandle.Alloc(fn);
        var thread = SDL_CreateThreadRuntime(&ThreadFunctionThunk, name, GCHandle.ToIntPtr(handle), 0, 0);
        if (thread == 0) handle.Free();
        return thread;
    }
    ...
    [LibraryImport(SDLLibrary, EntryPoint = "SDL_CreateThreadWithPropertiesRuntime"),
     UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    public static partial nint CreateThreadWithPropertiesRuntime(PropertiesID props,
    ...
    /// <para>
    /// This thread identifier is as reported by the underlying operating system. If SDL is running on a platform that does
    /// not support threads the return value will always be zero.
    /// </para>
    ...
    /// <para>
    /// Note that some platforms will not let you alter the priority (or at least, promote the thread to a higher priority)
    /// at all, and some require you to be an administrator account. Be prepared for this to fail.
    /// </para>
    /// </summary>
    ...
    /// <para>
    /// Once a thread has been cleaned up through this function, the SDL_Thread that references it becomes invalid and should
    /// not be referenced again. As such, only one thread may call <see cref="WaitThread"/> on another.
    /// </para>
    /// <para>
    /// The return code from the thread function is placed in the area pointed to by <c> status </c>, if <c> status </c> is
    /// not <c> null </c>.
    /// </para>
    /// <para>
    /// You may not wait on a thread that has been used in a call to <see cref="DetachThread"/>. Use either that function or
    /// this one, but not both, or behavior is undefined.
    /// </para>
    ...
    /// <see cref="WaitThread"/> on it. Detaching a thread is useful for long-running threads that nothing needs to synchronize with
    /// or further manage. When a detached thread is done, it simply goes away.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// Once a thread is detached, you should usually assume the SDL_Thread isn't safe to reference again, as it will become
    /// invalid immediately upon the detached thread's exit, instead of remaining until someone has called <see cref="WaitThread"/>
    ...
    /// </para>
    /// <para>
    /// You may not call <see cref="WaitThread"/> on a thread that has been detached. Use either that function or this one,
    /// but not both, or behavior is undefined.
    /// </para>
    ...
    /// <para>
    /// If the thread local storage ID is not initialized (the value is 0), a new ID will be created in a thread-safe way, so
    /// all calls using a pointer to the same ID will refer to the same local storage.
    /// </para>
    /// <para>
