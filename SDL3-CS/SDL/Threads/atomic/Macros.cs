...
public static partial class SDL
{
    ...
    ///     <para>
    ///         <b> Note: If you don't know what this macro is for, you shouldn't use it! </b>
    ///     </para>
    /// </summary>
    ...
    /// <returns> <c> true </c> if the variable reached zero after decrementing, <c> false </c> otherwise. </returns>
    /// <threadsafety> It is safe to call this macro from any thread. </threadsafety>
    /// <since> This macro is available since SDL 3.2.0 </since>
    ...
    ///     <para>
    ///         Memory barriers are designed to prevent reads and writes from being reordered by the compiler and being seen out
    ///         of order on multi-core CPUs.
    ///     </para>
    ///     <para>
    ///         A typical pattern would be for thread A to write some data and a flag, and for thread B to read the flag and get
    ///         the data. In this case you would insert a release barrier between writing the data and the flag, guaranteeing that
    ///         the data write completes no later than the flag is written, and you would insert an acquire barrier between reading
    ///         the flag and reading the data, to ensure that all the reads associated with the flag have completed.
    ///     </para>
    ///     <para>
    ///         In this pattern you should always see a release barrier paired with an acquire barrier and you should gate the
    ///         data reads/writes with a single flag variable.
    ///     </para>
    ///     <para>
    ...
    ///         <see cref="MemoryBarrierReleaseFunction"/> to do the heavy lifting. Apps that can use the macro should favor it over
    ///         the function.
    ///     </para>
    /// </summary>
    /// <threadsafety>
    ///     Obviously this macro is safe to use from any thread at any time, but if you find yourself needing this, you
    ///     are probably dealing with some very sensitive code; be careful!
    /// </threadsafety>
    /// <since> This macro is available since SDL 3.2.0 </since>
    ...
    ///     <para> Please see <see cref="MemoryBarrierRelease"/> for the details on what memory barriers are and when to use them. </para>
    ///     <para>
    ///         This is the macro version of this functionality; if possible, SDL will use compiler intrinsics or inline
    ///         assembly, but some platforms might need to call the function version of this,
    ///         <see cref="MemoryBarrierAcquireFunction"/>, to do the heavy lifting. Apps that can use the macro should favor it
    ///         over the function.
    ///     </para>
    /// </summary>
    /// <threadsafety>
